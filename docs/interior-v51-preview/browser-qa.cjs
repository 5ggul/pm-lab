...
async function runPriority(browser,report){
  ...
  await seedQuote(page);await page.click('[data-refresh-report]');await sleep(100);assert.equal(await nextId(page),'compare');
  ...
  const qCount=await completeQuestions(page);assert.ok(qCount>0);await page.click('[data-refresh-report]');await sleep(120);model=await page.evaluate(()=>window.InteriorQuoteReview51.buildModel());assert.equal(model.questionDone,model.questionTotal);assert.ok(model.candidateTotal>0,'expected reflection candidates from completed answers');assert.equal(model.next.id,'reflection');

  const cCount=await completeReflection(page);assert.ok(cCount>0);await page.click('[data-refresh-report]');await sleep(120);model=await page.evaluate(()=>window.InteriorQuoteReview51.buildModel());assert.equal(model.reflected,model.candidateTotal);assert.equal(model.next.id,'baseline');
