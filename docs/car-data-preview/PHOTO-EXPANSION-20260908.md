...
Photo coverage is 371 of 422 normalized model families. There are 362 unique credited source photographs; aliases and transmission/wheel entries can share a representative photograph. 51 families still have no approved photo. The latest pass consolidates MCPURA and MCPURA Cielo under one model family, preserves the former Cielo link, adds people-free KGM Musso Q300, AD Motors Change EV, and Renault Filante photographs, and replaces the raw Filante specification string with the model name 필랑트.
...
`data/photo-expansion-audit.json` records the current mappings and held candidates. All 4,203 source rows and catalogue record IDs remain intact. MCPURA coupe and Cielo source rows remain separately visible inside the consolidated family detail, and the Cielo body style is disclosed in the photo caption.
