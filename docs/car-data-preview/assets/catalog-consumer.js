(function(){
  ...
  function generationLabel(f){
    const labels=(f.generation_labels||[]).filter(v=>v&&!/미분류|확인 중|후보|개조 계열/.test(v));
  ...
  function representative(f){
    ...
    const reviewed=(window.CAR_CATALOG?.cars||[]).find(car=>f.path&&String(car.path||'').replace(/^\.\//,'')===f.path);
    if(reviewed?.rep){const rep=reviewed.rep;return {raw_model:rep.label,powertrain:rep.fuelType==='ev'?'electric':rep.fuelType,combined_efficiency:rep.combined,displacement_cc:rep.cc,range_km:rep.range,energy_cost_ready:rep.annualEnergy!=null,tax_ready:rep.tax!=null,full_cost_ready:rep.total!=null,fuel_price_key:rep.fuelType==='hybrid'?'gasoline':rep.fuelType};}
  ...
  function card(f,index){
    ...
    return `<article class="vehicle-card" data-family-id="${esc(f.family_id)}">${media(f,index)}<div class="vehicle-card-main"><div class="vehicle-card-maker">${esc(f.maker)}${category?' · '+esc(category):''}</div><h2>${esc(f.family_name)}</h2><div class="vehicle-card-meta">${esc(generationLabel(f))} · 신고 사양 ${Number(f.record_count||0).toLocaleString('ko-KR')}개</div><div class="vehicle-card-pills"><span class="vehicle-card-pill origin">${esc(originLabel(f))}</span>${pills}${more}</div><div class="vehicle-card-status">${decisionFacts(f)}</div><div class="card-scope">등록 사양 범위 · 연식별 차이</div><div class="card-availability">세금·연료·충전비 ${costLabel(f)} · 제조사 제원 ${spec}</div></div><div class="vehicle-card-actions">${detailAction}${costAction}<button type="button" data-compare-pick data-compare-mode="all" data-compare-id="${esc(f.family_id)}" data-compare-label="${esc(f.family_name)}">비교에 담기</button></div></article>`;
  }
