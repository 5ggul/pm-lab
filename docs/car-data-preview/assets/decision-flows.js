(function(){
 ...
 function render(sync=false){
  ...
  const c=CAR_DECISION_MATH.compare(p.left,p.right,km.valueAsNumber,price.valueAsNumber),difference=displayedDifference(c);$('decision-a').textContent=fmt(c.a.total);$('decision-b').textContent=fmt(c.b.total);$('decision-saving').textContent=data.kind==='hybrid'?(difference>=0?'하이브리드가 연 '+fmt(difference)+' 적음':'하이브리드가 연 '+fmt(-difference)+' 더 듦'):label(difference);
