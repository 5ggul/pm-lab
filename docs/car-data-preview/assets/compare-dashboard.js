(() => {
  ...
  function stacked(data) {
    ...
    return `<section class="compare-graphic compare-total" aria-label="연간 총비용 비교"><h3>연간 총비용 <small>연료·충전비 + 자동차세</small></h3><div class="compare-legend"><span><i class="chart-key-a"></i>A ${text(data.names[0])}</span><span><i class="chart-key-b"></i>B ${text(data.names[1])}</span></div><div class="compare-zero">0</div>${data.totals.map((total, index) => {
      const energy = data.energy[index], tax = data.tax[index];
      return `<div class="compare-total-row side-${index}"><span class="compare-side">${side(index)}</span><div class="compare-total-track" role="img" aria-label="${side(index)} 연료·충전비 ${money(energy)}, 자동차세 ${money(tax)}, 합계 ${money(total)}"><div class="compare-total-fill" style="width:${total / max * 100}%"><span class="energy-part" style="width:${energy / total * 100}%"></span><span class="tax-part" style="width:${tax / total * 100}%"></span></div></div><strong>${tenThousand(total)} <small>${money(total)}</small></strong></div>`;
    }).join('')}${Math.round(data.totals[0]) === Math.round(data.totals[1]) ? '<p class="compare-equal">차이 0원</p>' : ''}<p class="compare-chart-note">막대 안 진한 부분은 연료·충전비, 옅은 부분은 자동차세입니다.</p></section>`;
  }

  function components(data) {
    const items = [{title:'연료·충전비', values:data.energy}, {title:'자동차세', values:data.tax}];
  ...
  function distanceChart(data) {
    ...
    return `<section class="compare-graphic compare-distance" aria-label="주행거리별 총비용 비교"><h3>주행거리별 총비용</h3><svg viewBox="0 0 640 216" role="img" aria-label="연간 주행거리별 자동차세와 연료·충전비 합계. 검정 A, 파랑 B.">${grids}${paths}${dots}${ticks}</svg><div class="compare-distance-data">${exact}</div>${cross ? `<p class="compare-chart-note">약 ${cross.toLocaleString('ko-KR')} km에서 두 비용이 교차합니다.</p>` : ''}</section>`;
  }
  ...
  function render(data) {
    ...
    if ([...data.energy,...data.tax,...data.totals].some(value => !Number.isFinite(value))) {
      ...
      host.innerHTML = `${efficiency}<p class="compare-empty">비용 그래프는 두 사양의 자동차세와 연료·충전비가 모두 계산될 때 표시합니다. 전기차는 충전단가를 입력하세요.</p>`;
      return;
    }
    ...
    host.innerHTML = `<p class="compare-summary">연 ${data.km.toLocaleString('ko-KR')} km · A 대비 B 자동차세+연료·충전비: <strong>${result}</strong></p><div class="compare-chart-grid">${stacked(data)}${components(data)}</div>${distanceChart(data)}`;
  }
