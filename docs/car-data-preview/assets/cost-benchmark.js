(function(){
  ...
  function bind(data){rows=data?.benchmark_rows||[];update();['sourceRow','generation','familySearch'].forEach(id=>document.getElementById(id)?.addEventListener('change',update));['price','km','familySearch'].forEach(id=>document.getElementById(id)?.addEventListener('input',update));document.addEventListener('car-cost-context-change',update)}
