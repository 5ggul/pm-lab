(function(){
  ...
  function update(){
    ...
    if(!selected||!selected.full_cost_ready||!(distance>=1000&&distance<=100000&&price>0&&selected.combined_efficiency>0)){clear();return}
  ...
  fetch('../../data/generated/all-car-calc-index.json',{cache:'no-cache'}).then(r=>r.json()).then(data=>{rows=data.rows||[];update();['sourceRow','generation','familySearch'].forEach(id=>document.getElementById(id)?.addEventListener('change',update));['price','km','familySearch'].forEach(id=>document.getElementById(id)?.addEventListener('input',update));document.addEventListener('car-cost-context-change',update)}).catch(clear);
})();
