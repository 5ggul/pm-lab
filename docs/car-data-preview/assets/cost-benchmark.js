(function(){
  ...
  function update(){
    const mode=document.documentElement.dataset.costMode||'all';host.hidden=mode!=='all';if(mode!=='all')return;
    const select=document.getElementById('sourceRow'),selected=rows.find(row=>row.calc_id===select?.value);
    const distance=Number(document.getElementById('km')?.value||0),price=Number(document.getElementById('price')?.value||0);
    if(!selected||!selected.full_cost_ready||!(distance>0&&price>0&&selected.combined_efficiency>0)){clear();return}
  ...
  fetch('../../data/generated/all-car-calc-index.json',{cache:'no-cache'}).then(r=>r.json()).then(data=>{rows=data.rows||[];update();['sourceRow','generation'].forEach(id=>document.getElementById(id)?.addEventListener('change',update));['price','km','familySearch'].forEach(id=>document.getElementById(id)?.addEventListener('input',update));document.addEventListener('car-cost-context-change',update)}).catch(clear);
})();
