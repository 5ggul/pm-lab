...
export function photoMarkup(f,record,detail=false,priority=false){
  ...
  return `<figure class="vehicle-photo${detail?' family-photo':''}" data-photo-family="${esc(f.family_id)}"><div class="vehicle-card-media">${placeholder}${pictureStart}<img src="${esc(src)}" alt="${esc(photoAlt)}" width="${record.width}" height="${record.height}" loading="${detail||priority?'eager':'lazy'}" fetchpriority="${detail||priority?'high':'low'}" decoding="async">${variants.length?"</picture>":""}</div><figcaption><span class="vehicle-photo-generation">사진: ${esc(record.generation)}</span><a class="vehicle-card-credit" href="${esc(record.source_page)}" target="_blank" rel="noopener noreferrer">${esc(record.author)} · ${esc(record.license)}</a> · <a class="vehicle-photo-license" href="${esc(record.license_url)}" target="_blank" rel="noopener noreferrer">이용 조건</a><span class="vehicle-photo-note">${esc(record.display_note||'연식·트림에 따라 외관 차이')}</span></figcaption></figure>`;
}
