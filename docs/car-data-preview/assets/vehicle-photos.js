...
export function photoMarkup(f,record,detail=false,priority=false){
  ...
  return `<figure class="vehicle-photo${detail?' family-photo':''}" data-photo-family="${esc(f.family_id)}"><div class="vehicle-card-media">${placeholder}${pictureStart}<img src="${esc(src)}" alt="${esc(photoAlt)}" width="${record.width}" height="${record.height}" loading="${detail||priority?'eager':'lazy'}" fetchpriority="${detail||priority?'high':'low'}" decoding="async">${variants.length?"</picture>":""}</div><figcaption>${detail?attribution:`<details><summary>사진 출처</summary>${attribution}</details>`}</figcaption></figure>`;
}
