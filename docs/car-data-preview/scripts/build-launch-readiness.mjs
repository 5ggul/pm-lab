...
write('tools',page('자동차 계산 도구','자동차세, 유류비, 충전비를 필요한 항목별로 계산하세요.','tools',section(`<div class="utility-directory">${[...tools,{slug:'annual-cost',title:'1년 유지비 계산기',description:'차량 사양을 선택해 자동차세와 에너지비를 함께 계산합니다.'}].map(t=>`<a href="./${t.slug}/"><h2>${t.title}</h2><p>${t.description}</p><span>계산하기 →</span></a>`).join('')}</div>`)));
for(const guide of guides){
  ...
  write(`guide/${guide.slug}`,page(guide.title,guide.summary,`guide/${guide.slug}`,section(`<article class="utility-article">${guide.body}<div class="utility-reading"><h2>직접 계산하기</h2><a href="../../tools/${guide.tool}/">${guide.tool==='annual-cost'?'1년 유지비 계산기':tools.find(t=>t.slug===guide.tool).title} →</a><p>관련 공식 자료: ${sourceLinks}</p><p class="notice">기준 확인 2026.09.07 · 내차데이터</p><a href="../">다른 가이드 보기 →</a></div></article>`)));
}
