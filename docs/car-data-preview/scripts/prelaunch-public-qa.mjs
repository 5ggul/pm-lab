...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(home,/<main class="editorial-home">/);assert.match(home,/<h1>차량 찾기부터 비교·계산·순위·리콜까지<\/h1>/);assert.match(home,/class="home-hero-visual"/);assert.doesNotMatch(home,/공식 신고 사양 · 연 20,000 km 에너지비|차량 사양을 고르면 연비·전비/);assert.equal((home.match(/class="home-car"/g)||[]).length,6);assert.match(home,/3,826,211원/);assert.match(home,/1,470,331원/);assert.doesNotMatch(home,/<small>0[123]<\/small>|hero-data-stream/);
