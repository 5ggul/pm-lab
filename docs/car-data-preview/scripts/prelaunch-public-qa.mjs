...
for(const car of catalog.cars.filter(row=>row.indexable&&row.energy!=='ev')){const html=fs.readFileSync(path.join(root,car.path.slice(2),'index.html'),'utf8'),metric=html.match(/<div class="model-metric"><small>세금\+(?:유류비|충전비|연료·충전비)<\/small><b[^>]*>([\d,]+)원<\/b>/)?.[1];if(metric)assert.equal(Number(metric.replaceAll(',','')),car.rep.total,`${car.id} hero total is stale`)}
for(const file of fs.readdirSync(path.join(root,'rankings')).filter(name=>fs.existsSync(path.join(root,'rankings',name,'index.html')))){const html=fs.readFileSync(path.join(root,'rankings',file,'index.html'),'utf8');assert.doesNotMatch(html,/cars\/record\/\?id=/,`${file} links generic record route`);for(const article of html.matchAll(/<article class="rank-row"[\s\S]*?<\/article>/g)){const calcId=article[0].match(/data-calc-id="([^"]+)"/)?.[1],shown=article[0].match(/<p[^>]*>([^<]*)<\/p>/)?.[1]||'',seat=shown.match(/(\d{1,2})인승/)?.[1],source=calc.rows.find(row=>row.calc_id===calcId);if(seat)assert.match(source?.raw_model||'',new RegExp(`${seat}\\s*인승`),`${file} derives ${seat} seats from a non-seat token`)}}
...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8'),homeCatalog=JSON.parse(fs.readFileSync(path.join(root,'data/generated/catalog.json'),'utf8')),homeGrandeurTotal=homeCatalog.cars.find(car=>car.id==='grandeur-gn7').rep.total.toLocaleString('ko-KR')+'원',homeHybridComparison=fs.readFileSync(path.join(root,'compare/grandeur-gasoline-vs-hybrid/index.html'),'utf8').match(/id="decision-saving">[^<]*?([\d,]+원)/)?.[1];assert.match(home,/<main class="editorial-home">/);assert.match(home,/<h1>차량 찾기부터 비교·계산·순위·리콜까지<\/h1>/);assert.match(home,/class="home-hero-visual"/);assert.doesNotMatch(home,/공식 신고 사양 · 연 20,000 km 연료·충전비|차량 사양을 고르면 연비·전비/);assert.equal((home.match(/class="home-car"/g)||[]).length,6);assert(home.includes(homeGrandeurTotal));assert(homeHybridComparison&&home.includes(homeHybridComparison));assert.doesNotMatch(home,/<small>0[123]<\/small>|hero-data-stream/);
const compareHub=fs.readFileSync(path.join(root,'compare/index.html'),'utf8');assert.match(compareHub,/compare\/grandeur-vs-k8\//,'compare hub omits Grandeur vs K8');assert.equal((compareHub.match(/compare-presets/g)||[]).length,1,'comparison presets duplicated or missing');assert.ok(compareHub.indexOf('compare-presets')>compareHub.indexOf('tool-grid'),'comparison presets must follow the A/B comparator');assert.equal((compareHub.match(/page-fuel-status-foot/g)||[]).length,1,'fuel status duplicated');assert.ok(compareHub.indexOf('page-fuel-status-foot')>compareHub.indexOf('compare-presets'),'fuel status must be last in comparison main');
const rayCompare=fs.readFileSync(path.join(root,'compare/ray-vs-casper/index.html'),'utf8');assert.equal((rayCompare.match(/class="decision-photo-credit"/g)||[]).length,2);assert.match(rayCompare,/class="static-compare-summary"/);assert.match(rayCompare,/class="compare-distance-chart"/);assert.ok((rayCompare.match(/class="metric-chart/g)||[]).length>=1,'static comparison omits efficiency chart');assert.doesNotMatch(rayCompare,/metric-waterfall/);
const fuelTool=fs.readFileSync(path.join(root,'tools/fuel-cost/index.html'),'utf8');assert.ok(fuelTool.indexOf('tool-fuel-status-foot')>fuelTool.indexOf('utility-faq'),'fuel refresh status must follow calculator content');
...
const grandeurQaCar=catalog.cars.find(car=>car.id==='grandeur-gn7'),grandeurVariant=grandeurQaCar.variants[0],grandeurTax=Math.round(grandeurVariant.cc*200*1.3),grandeurEnergy=Math.round(20000/grandeurVariant.combined*fuel.prices.gasoline),money=value=>value.toLocaleString('ko-KR')+'원';
await interactive.goto(new URL('tools/annual-cost/?car=grandeur-gn7',base).href,{waitUntil:'networkidle'});assert.equal(await interactive.locator('#car').inputValue(),'grandeur-gn7');assert.equal(await interactive.locator('#tax').innerText(),money(grandeurTax));assert.equal(await interactive.locator('#energy').innerText(),money(grandeurEnergy));assert.equal(await interactive.locator('#total').innerText(),money(grandeurTax+grandeurEnergy));
