...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8'),homeCatalog=JSON.parse(fs.readFileSync(path.join(root,'data/generated/catalog.json'),'utf8')),homeGrandeurTotal=homeCatalog.cars.find(car=>car.id==='grandeur-gn7').rep.total.toLocaleString('ko-KR')+'원',homeHybridComparison=fs.readFileSync(path.join(root,'compare/grandeur-gasoline-vs-hybrid/index.html'),'utf8').match(/id="decision-saving">[^<]*?([\d,]+원)/)?.[1];assert.match(home,/<main class="editorial-home">/);assert.match(home,/<h1>차량 찾기부터 비교·계산·순위·리콜까지<\/h1>/);assert.match(home,/class="home-hero-visual"/);assert.doesNotMatch(home,/공식 신고 사양 · 연 20,000 km 에너지비|차량 사양을 고르면 연비·전비/);assert.equal((home.match(/class="home-car"/g)||[]).length,6);assert(home.includes(homeGrandeurTotal));assert(homeHybridComparison&&home.includes(homeHybridComparison));assert.doesNotMatch(home,/<small>0[123]<\/small>|hero-data-stream/);
