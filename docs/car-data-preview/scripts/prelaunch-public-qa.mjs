...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(home,/<main class="editorial-home">/);assert.match(home,/차 사기 전에 연비와 세금을 비교하세요/);assert.equal((home.match(/class="home-car"/g)||[]).length,6);assert.doesNotMatch(home,/<small>0[123]<\/small>|hero-data-stream/);
