...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(home,/<main class="editorial-home">/);assert.match(home,/<h1>차 고를 때 보는 연비·자동차세<\/h1>/);assert.doesNotMatch(home,/공식 신고 사양 · 연 20,000 km 에너지비/);assert.equal((home.match(/class="home-car"/g)||[]).length,6);assert.match(home,/3,826,211원/);assert.match(home,/1,470,331원/);assert.doesNotMatch(home,/<small>0[123]<\/small>|hero-data-stream/);
const compareHub=fs.readFileSync(path.join(root,'compare/index.html'),'utf8');assert.match(compareHub,/compare\/grandeur-vs-k8\//,'compare hub omits Grandeur vs K8');
