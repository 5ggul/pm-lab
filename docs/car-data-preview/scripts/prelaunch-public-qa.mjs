...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(home,/<main class="editorial-home">/);assert.match(home,/<h1>차량별 연비·전비·자동차세·연료비 비교<\/h1>/);assert.doesNotMatch(home,/공식 신고 사양 · 연 20,000 km 에너지비/);assert.equal((home.match(/class="home-car"/g)||[]).length,6);assert.doesNotMatch(home,/<small>0[123]<\/small>|hero-data-stream/);
