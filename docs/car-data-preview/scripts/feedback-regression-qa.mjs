...
assert.doesNotMatch(familyHtml,/(?:Ford Explorer|Lincoln Aviator)<\/strong> · 2\.4 km\/L/);
...
const expectedEnergyUnit={gasoline:'km/L',diesel:'km/L',lpg:'km/L',hybrid:'km/L',electric:'km/kWh'};
assert.equal(calcIndex.rows.filter(row=>row.energy_cost_ready&&row.efficiency_unit!==expectedEnergyUnit[row.powertrain]).length,0,'energy-ready rows must use a compatible official efficiency unit');
assert(calcIndex.rows.some(row=>/카이엔 .*E-하이브리드/i.test(row.raw_model)&&row.powertrain==='phev'&&!row.energy_cost_ready&&!row.fuel_price_key),'Porsche E-Hybrid rows must not use a gasoline-only cost formula');
assert(calcIndex.rows.filter(row=>row.powertrain==='phev').every(row=>!row.energy_cost_ready&&!row.fuel_price_key),'PHEV rows must stay excluded until electric/fuel usage share is available');
const changeRows=calcIndex.rows.filter(row=>row.family_name==='Change');assert(changeRows.length&&changeRows.every(row=>row.powertrain==='electric'&&row.efficiency_unit==='km/kWh'&&row.energy_cost_ready),'Change electric rows must use km/kWh and remain calculable');
const familyDetailIndex=JSON.parse(fs.readFileSync(path.join(root,'data/generated/family-detail-index.json'),'utf8'));
const carsHtml=fs.readFileSync(path.join(root,'cars/index.html'),'utf8');assert.doesNotMatch(carsHtml,/\d+종 신고 사양/);assert.match(carsHtml,new RegExp(`차량 ${familyDetailIndex.family_count}종`));
assert(carsHtml.indexOf('assets/cost-math.js')>=0&&carsHtml.indexOf('assets/cost-math.js')<carsHtml.indexOf('assets/catalog-consumer.js'),'catalog cost math must load before the card renderer');
...
try{
  ...
  await page.waitForFunction(()=>document.querySelector('#compareTable')?.textContent?.includes('세금 + 선택 주행거리 연료·충전비'));
  ...
  const bmw330eElectric=calcRows.find(row=>row.calc_id==='kea-display-37f332d6d65f0175df');
  assert.equal(bmw330eElectric?.powertrain,'phev');assert.equal(bmw330eElectric?.energy_cost_ready,false);assert.equal(bmw330eElectric?.fuel_price_key,null);assert.equal(bmw330eElectric?.displacement_cc,1998);assert.equal(bmw330eElectric?.tax_ready,true);
  const changeRow=calcRows.find(row=>row.family_id==='family-327d41d5ed75df91'&&row.raw_model==='Change');assert.equal(changeRow?.powertrain,'electric');assert.equal(changeRow?.efficiency_unit,'km/kWh');assert.equal(changeRow?.energy_cost_ready,true);
  ...
  assert.match(await page.locator('#familySearch').inputValue(),/넥쏘/);assert.equal(await page.locator('#priceLabelText').textContent(),'수소 단가 자동 계산 제외');assert.equal(await page.locator('#price').isDisabled(),true);assert.match(await page.locator('#sourceRow option').first().textContent(),/km\/kg/);assert.match(await page.locator('#detailLink').getAttribute('href'),/family\/\?id=hyundai-nexo/);assert.equal(await page.locator('#tax').textContent(),'130,000원');assert.equal(await page.locator('#total').textContent(),'연료·충전비 계산 제외');assert.equal(await page.locator('[data-cost-benchmark]').locator('xpath=..').isHidden(),true);
  await page.goto(base+'/cars/family/?id=hyundai-nexo');await page.waitForFunction(()=>document.querySelector('.calc-strip')?.textContent?.includes('자동차세 계산 가능'));assert.match(await page.locator('.calc-strip').innerText(),/자동차세 계산 가능[\s\S]*수소 연료·충전비는 자동 계산에서 제외/);
  await page.goto(base+`/tools/annual-cost/?mode=all&fa=${encodeURIComponent(bmw330eElectric.family_id)}&calc=${encodeURIComponent(bmw330eElectric.calc_id)}&km=20000`);await page.waitForFunction(id=>document.querySelector('#sourceRow')?.value===id,bmw330eElectric.calc_id);assert.match(await page.locator('#sourceRow option:checked').innerText(),/플러그인 하이브리드[\s\S]*3\.3 km\/kWh/);assert.equal(await page.locator('#tax').textContent(),'519,480원');assert.equal(await page.locator('#energy').textContent(),'계산 제외');assert.equal(await page.locator('#total').textContent(),'연료·충전비 계산 제외');assert.equal(await page.locator('#price').isDisabled(),true);assert.equal(await page.locator('[data-cost-benchmark]').locator('xpath=..').isHidden(),true);
  await page.goto(base+`/cars/family/?id=${encodeURIComponent(changeRow.family_id)}`);await page.waitForFunction(()=>document.querySelector('[data-family-universal="ready"]'));assert.match(await page.locator('main').innerText(),/4\.3 km\/kWh/);assert.doesNotMatch(await page.locator('main').innerText(),/4\.3 km\/L/);
  const nexoRow=nexoCalcRows[0],sorentoRow=calcRows.find(row=>row.family_id==='kia-sorento'&&row.full_cost_ready);assert(sorentoRow);
  await page.goto(`${base}/compare/?fa=hyundai-nexo&ra=${encodeURIComponent(nexoRow.calc_id)}&fb=kia-sorento&rb=${encodeURIComponent(sorentoRow.calc_id)}&km=20000`);await page.waitForFunction(()=>document.querySelector('#compareTable')?.textContent?.includes('자동차세'));assert.match(await page.locator('#compareTable').innerText(),/130,000원/,'Nexo comparison must use the fixed passenger-car tax');assert.match(await page.locator('#compareTable').innerText(),/96\.2 km\/kg/);
  ...
  console.log('PASS semantic unit gate, PHEV exclusion, Nexo tax across calculator/family/compare, JSON rebuild input, state synchronization and ranking safeguards.');
