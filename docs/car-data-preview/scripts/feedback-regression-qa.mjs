...
try{
  ...
  await page.goto(base+'/cars/family/?id=hyundai-nexo');await page.waitForFunction(()=>document.querySelector('.calc-strip')?.textContent?.includes('자동차세 계산 가능'));assert.match(await page.locator('.calc-strip').innerText(),/자동차세 계산 가능[\s\S]*수소 에너지비는 자동 계산에서 제외/);
  await page.goto(base+`/tools/annual-cost/?mode=all&fa=${encodeURIComponent(bmw330eElectric.family_id)}&calc=${encodeURIComponent(bmw330eElectric.calc_id)}&km=20000`);await page.waitForFunction(id=>document.querySelector('#sourceRow')?.value===id,bmw330eElectric.calc_id);assert.match(await page.locator('#sourceRow option:checked').innerText(),/플러그인 하이브리드[\s\S]*3\.3 km\/kWh/);assert.equal(await page.locator('#tax').textContent(),'519,480원');assert.equal(await page.locator('#energy').textContent(),'계산 제외');assert.equal(await page.locator('#total').textContent(),'에너지비 계산 제외');assert.equal(await page.locator('#price').isDisabled(),true);assert.equal(await page.locator('[data-cost-benchmark]').locator('xpath=..').isHidden(),true);
