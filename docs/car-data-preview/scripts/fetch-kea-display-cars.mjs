...
try{const text=await requestText(csvUrl,{timeout:65000,attempts:3});const rows=parseCsv(text);if(!rows.length)throw new Error('CSV returned zero rows');const p=persist(rows,{transport:'csv',sourceUrl:csvSourcePage});status({ok:true,status:nativeResult?'fetched_native_api_and_csv':'fetched_csv_fallback',transport:nativeResult?'api+csv':'csv',fetched_at:fetchedAt,source_url:csvSourcePage,csv_url:csvUrl,api_source_url:nativeSourcePage,api_endpoint:nativeEndpoint,api_rows:nativeResult?.rows.length||0,api_total_count:nativeResult?.total||0,rows:p.count,usable_rows:p.usable,key_slot:nativeResult?.keySlot||null,native_api_error:nativeFailure?.slice(0,500)||null});console.log(`KEA display CSV fetched ${p.count} rows${nativeResult?` after verifying ${nativeResult.rows.length} live API rows`:''}`)}catch(e){const csvError=redact(e?.message||e);status({ok:false,status:'fetch_failed',fetched_at:fetchedAt,source_url:csvSourcePage,native_api_error:nativeFailure?.slice(0,500)||null,csv_error:csvError.slice(0,500),api_rows:nativeResult?.rows.length||0,snapshot_exists:fs.existsSync(rawPath)});console.error(`KEA display source failed: ${csvError}`);process.exitCode=2}
