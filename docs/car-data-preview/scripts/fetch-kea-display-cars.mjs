...
function stableId(row){const base=`${pick(row,'COMP_NM','제조(수입사)','제조사','ENTP_NM','ENTP_NAME','MAKER','MANUFACTURER')||''}|${pick(row,'모델명','MODEL_NM','MODEL_NAME','CAR_NM','CAR_NAME','MODEL')||''}|${pick(row,'FUEL_NM','유형','FUEL','FUEL_TYPE','TYPE')||''}|${pick(row,'DISPLAY_EFF','복합_연비','복합연비','MILEAGE','FUEL_EFFICIENCY','COMB_EFF')||''}|${pick(row,'URBAN_EFF','도심_연비','도심연비','CITY_MILEAGE','CITY_EFF')||''}|${pick(row,'HIGHWAY_EFF','고속도로_연비','고속도로연비','HIGHWAY_MILEAGE','HWY_MILEAGE')||''}|${pick(row,'RANGE_PER_CHARGE','1회충전주행거리','1회충전거리','DRIVING_RANGE','RANGE_KM')||''}`;return 'kea-display-'+crypto.createHash('sha1').update(base).digest('hex').slice(0,18)}
...
function normalize(rows,transport,sourceUrl){return rows.map((r,i)=>{const rowTransport=r.__source_transport||transport;return ({
  ...
  source_row_index:i,source_dataset:rowTransport==='native_api'?'KEA_CAREFF_LIST':'KEA_DISPLAY_EFFICIENCY_20260424',transport:rowTransport,source_url:rowTransport==='native_api'?nativeSourcePage:sourceUrl,publishable:false,match_status:'unmatched'
})})}
function mergeRows(csvRows,apiRows){const merged=new Map();for(const row of csvRows)merged.set(stableId(row),{...row,__source_transport:'csv'});for(const row of apiRows)merged.set(stableId(row),{...row,__source_transport:'native_api'});return[...merged.values()]}
