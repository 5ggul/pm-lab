...
assert.ok(calc.tax_ready>=Math.ceil(calc.rows*.35)&&calc.tax_ready<=calc.rows,`tax-ready coverage out of bounds: ${calc.tax_ready}`);
assert.ok(calc.energy_ready>=Math.ceil(calc.rows*.55)&&calc.energy_ready<=calc.rows,`energy-ready coverage out of bounds: ${calc.energy_ready}`);
assert.ok(calc.full_ready>=Math.ceil(calc.rows*.25)&&calc.full_ready<=Math.min(calc.tax_ready,calc.energy_ready),`full-cost coverage out of bounds: ${calc.full_ready}`);
assert.ok(calc.electric>=Math.ceil(calc.rows*.10)&&calc.electric<=calc.rows,`electric coverage out of bounds: ${calc.electric}`);
