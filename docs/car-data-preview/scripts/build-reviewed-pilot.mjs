...
const credit=id=>{const r=reviewedImage(id),note=r.depicted_spec?`사진 속 사양: ${esc(r.depicted_spec)} · 연비·세금은 선택 사양 기준`:'연식·트림·동력 사양에 따라 외관이 다를 수 있습니다.';return `<span class="pilot-credit">사진: ${esc(r.generation)} · <a href="${esc(r.source_page)}">${esc(r.author)} · ${esc(r.license)}</a> · <a href="${esc(r.license_url)}">이용 조건</a><br>${note}</span>`;};
const pairs=[
  ...
  {slug:'grandeur-vs-k8',title:'그랜저와 K8 비교',a:'grandeur-gn7',av:'gn7-g25-2wd-18',b:'k8-gl3',bv:'k8-g25-2wd-17',note:'2.5 가솔린 · 2WD. 그랜저는 18인치, K8은 17인치 휠 기준입니다.'},
...
function variantCard(id,vid){const c=cars.find(c=>c.id===id),v=c?.variants.find(v=>v.id===vid);if(!c?.indexable||!v)throw Error(`Unreviewed comparison selection ${id}/${vid}`);return `<article data-pilot-car="${id}" data-pilot-variant="${vid}">${photo(id)}${credit(id)}<h2>${esc(comparisonLabel(c.model,c.energy==='ev'?'electric':v.fuelType))}</h2><p>${esc(c.yearLabel)} · ${esc(v.label)}</p><dl><dt>복합 ${c.energy==='ev'?'전비':'연비'}</dt><dd>${v.combined} ${c.energy==='ev'?'km/kWh':'km/L'}</dd><dt>${c.energy==='ev'?'1회 충전 주행거리':'배기량'}</dt><dd>${c.energy==='ev'?`${v.range} km`:`${v.cc.toLocaleString('ko-KR')} cc`}</dd><dt>전장 / 전폭</dt><dd>${c.dimensions.length_mm.toLocaleString('ko-KR')} / ${c.dimensions.width_mm.toLocaleString('ko-KR')} mm</dd><dt>축거</dt><dd>${c.dimensions.wheelbase_mm.toLocaleString('ko-KR')} mm</dd></dl><p class="pilot-note">차체 크기는 ${esc(c.code)} 제조사 제원 기준입니다. 확인일 ${esc(c.reviewedOn)}.</p><div class="pilot-links"><a href="${esc(c.sourceUrl)}">공식 연비·전비 자료</a><a href="${esc(c.specSourceUrl)}">제조사 제원</a><a href="../../${c.path.slice(2)}">차량 상세</a></div></article>`;}
for(const p of pairs){
  const params=new URLSearchParams({mode:'reviewed',a:p.a,av:p.av,b:p.b,bv:p.bv,km:'20000'});
  const html=`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><title>${p.title} | 내차데이터</title><meta name="description" content="${esc(p.note)}"><link rel="canonical" href="${siteConfig.baseUrl}compare/${p.slug}/"><link rel="stylesheet" href="../../assets/site.css"><link rel="stylesheet" href="../../assets/home.css"></head><body><header class="db-header"><div class="db-shell"><a class="db-logo" href="../../">내차데이터</a><nav class="db-nav" aria-label="주 메뉴"><a href="../../cars/">차량</a><a href="../">비교</a><a href="../../tools/annual-cost/">비용 계산</a><a href="../../recalls/">리콜</a><a href="../../data-sources/">출처</a></nav></div></header><main><section class="page-hero"><div class="pilot-wrap"><p><a href="../">차량 비교</a> / ${p.title}</p><h1>${p.title}</h1><p>${p.note}</p><a class="pilot-cta" data-pilot-calculate href="../?${esc(params)}">이 사양으로 세금·연료·충전비 계산</a><p class="pilot-note">연 20,000km로 시작하며 주행거리와 단가를 바꿀 수 있습니다. 전기차는 충전단가를 입력한 뒤 계산합니다.</p></div></section><section class="db-section"><div class="pilot-wrap pilot-grid">${variantCard(p.a,p.av)}${variantCard(p.b,p.bv)}</div></section><section class="pilot-trust"><div class="pilot-wrap"><h2>비교 전에 확인하세요</h2><p class="cost-scope-inline">포함: 자동차세 + 연료·충전비 · 제외: 구매가격·취득세·보험·정비·감가상각</p><details class="cost-basis"><summary>포함 항목과 계산식</summary><p>${scope}</p><p>${formula}</p></details><p>연비·전비는 위 사양 기준입니다.</p><div class="pilot-links"><a href="../../methodology/">계산 기준</a><a href="../../data-sources/">출처와 갱신 기준</a></div></div></section></main><footer class="db-footer"><div class="pilot-wrap">내차데이터 · 공식 자료와 입력 조건을 함께 확인하세요.</div></footer></body></html>`;
...
const links=`<section class="pilot-trust"><div class="pilot-wrap"><h2>사양을 맞춰 비교하기</h2><p>먼저 엔진·구동·휠 조건을 확인하고, 같은 주행거리로 자동차세와 연료·충전비를 비교하세요.</p><div class="pilot-links">${pairs.map(p=>`<a href="./compare/${p.slug}/">${p.title}</a>`).join('')}</div></div></section>`;
...
console.log(`Reviewed pilot: 6 details, ${pairs.length} comparison pages.`);
await import('./build-clear-experience.mjs');
