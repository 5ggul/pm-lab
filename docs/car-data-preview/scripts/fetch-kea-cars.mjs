...
async function getPage(pageNo){const url=new URL(endpoint);url.searchParams.set('serviceKey',safeKey(keyRaw));url.searchParams.set('pageNo',String(pageNo));url.searchParams.set('numOfRows',String(pageSize));url.searchParams.set('apiType','json');url.searchParams.set('type','json');const res=await fetch(url,{headers:{'user-agent':'pm-lab-car-preview/1.0'},signal:AbortSignal.timeout(25000)});const text=await res.text();let payload=null;try{payload=JSON.parse(text)}catch{}if(!res.ok)throw new Error(`HTTP ${res.status}: ${text.slice(0,500)}`);const err=apiError(payload,text);if(err)throw new Error(err);if(!payload)throw new Error(`Non-JSON response: ${text.slice(0,300)}`);return{payload,rows:itemsFrom(payload),total:totalFrom(payload)}}
async function getCsv(){const res=await fetch(csvUrl,{headers:{'user-agent':'pm-lab-car-preview/1.0'},redirect:'follow',signal:AbortSignal.timeout(45000)});if(!res.ok)throw new Error(`CSV HTTP ${res.status}`);const text=await res.text();const rows=parseCsv(text);if(!rows.length)throw new Error('CSV contains no rows');return rows}
