...
function normalizePublicHtml(){
  ...
  const walk=dir=>{for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    if(rel==='compare/index.html'){
      ...
      html=html.replace("mode==='all'?'전체 공식 사양 중 계산 조건이 확인된 항목만 금액 비교에 사용합니다.':'제조사 공식 제원이 있는 차량끼리 비교합니다.'","mode==='all'?'신고 사양 중 세금 또는 에너지비를 계산할 수 있는 항목입니다.':'제조사 제원이 연결된 대표 사양입니다.'");
