...
const manufacturerDetailCount=families.filter(row=>row.manufacturer_detail).length;
...
const annualTax=row=>['electric','hydrogen'].includes(row.powertrain)?130000:Number(row.displacement_cc)>0?Math.round(Number(row.displacement_cc)*(row.displacement_cc<=1000?80:row.displacement_cc<=1600?140:200)*1.3):null;
...
function normalizePublicHtml(){
  const banned=[
    ['Phase 1','현재 계산'],['GitHub Actions 정기 실행','정기 갱신'],['GitHub Actions','자동 갱신'],['품질 게이트','공개 전 확인'],['SEO 상세 페이지','차량 상세 페이지'],['세대 미분류','연식 통합'],['raw_only','source_only'],['reviewed_override','confirmed_mapping'],['snapshot','저장 자료'],['workflow','갱신 절차'],['차량을 선택하면 상세 사양이 표시됩니다.','차량명과 복합 효율은 아래 목록에서도 확인할 수 있습니다.'],['공식 신고 데이터','한국에너지공단 표시연비'],['어떤 기준으로 차를 찾고 있나요?','연비·전비·자동차세 순위'],['구매가격까지 비교','하이브리드 연료비 차이로 가격 차를 나눠 보기'],['전체 차량</button>','신고 사양 전체</button>'],['주요 차량</button>','제원 확인된 대표 사양</button>'],['가솔린와','가솔린과'],['디젤와','디젤과'],['LPG와','LPG와'],['공식 데이터 신고 데이터 보기','신고 원문 보기']
  ];
  const walk=dir=>{for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    html=html.replace(/제원 확인된 \d+종<\/button>/g,'제원 확인된 대표 사양</button>').replace(/제조사 제원이 연결된 \d+종입니다\./g,'제조사 제원이 연결된 대표 사양입니다.');
    ...
    if(rel==='compare/index.html'){
      ...
      html=html.replace("mode==='all'?'전체 공식 사양 중 계산 조건이 확인된 항목만 금액 비교에 사용합니다.':'제조사 공식 제원이 있는 차량끼리 비교합니다.'","mode==='all'?'신고 사양 중 세금 또는 연료·충전비를 계산할 수 있는 항목입니다.':'제조사 제원이 연결된 대표 사양입니다.'");
    ...
    if(rel==='tools/annual-cost/index.html'){
      html=html.replace("if(fuelKey==='electric')return{auto:100000,edu:30000,total:130000,discount:0}","if(fuelKey==='electric'||fuelKey==='hydrogen')return{auto:100000,edu:30000,total:130000,discount:0}");
      ...
      html=html.replace("reg.addEventListener('change',()=>mode==='all'?renderAll():renderReviewed());","reg.addEventListener('change',()=>{mode==='all'?renderAll():renderReviewed();document.dispatchEvent(new CustomEvent('car-cost-context-change',{detail:{mode}}))});");
      ...
      if(!html.includes('data-cost-benchmark'))html=html.replace('</main>',`<section class="db-section" hidden><div class="db-shell"><div class="cost-benchmark" data-cost-benchmark hidden><h2>같은 동력 사양과 비교</h2><p data-benchmark-scope>${bench?`${bench.vehicleClass} · ${ptLabel[bench.powertrain]} · 계산 가능한 신고 사양 ${bench.count}개 · 신차 세액 기준`:'차량을 선택하면 비교 범위가 표시됩니다.'}</p><div class="benchmark-values"><div><span>선택 사양</span><strong data-benchmark-current>${bench?money(bench.current):'—'}</strong></div><div><span>중앙값</span><strong data-benchmark-median>${bench?money(bench.mid):'—'}</strong></div><div><span>평균</span><strong data-benchmark-average>${bench?money(bench.avg):'—'}</strong></div></div><div class="benchmark-range"><div class="benchmark-track"><span data-benchmark-fill style="width:${bench?bench.percent:0}%"></span></div><div class="benchmark-rank"><b data-benchmark-diff>${bench?`${bench.current<=bench.mid?'중앙값보다 낮음':'중앙값보다 높음'} ${money(Math.abs(bench.current-bench.mid))}`:'—'}</b><span data-benchmark-rank>${bench?`낮은 비용부터 ${bench.rank} / ${bench.count} · ${bench.percent}% 위치`:'—'}</span></div></div></div></div></section></main>`).replace('</body>','<script src="../../assets/cost-benchmark.js"></script></body>');
      html=html.replace(/<section class="db-section"(?: hidden)?><div class="db-shell"><div class="cost-benchmark" data-cost-benchmark(?: hidden)?>/,'<section class="db-section" hidden><div class="db-shell"><div class="cost-benchmark" data-cost-benchmark hidden>');
    }
    if(/^cars\/(?:hyundai|kia|genesis)\/[^/]+\/index\.html$/.test(rel))html=html.replace(/(<small>세금\+(?:유류비|충전비|연료·충전비)<\/small><b)(?![^>]*data-field)/g,'$1 data-field="annual-total"');
