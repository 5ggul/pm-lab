...
import {isPriceStale} from './fuel-price-state.mjs';
...
const staleSince=isPriceStale;

if(!key){const prev=current();if(prev){prev.stale=true;fs.writeFileSync(dataPath,JSON.stringify(prev,null,2)+'\n')}writeStatus({ok:false,status:'missing_api_key',checked_at:now.toISOString(),endpoint,snapshot_exists:Boolean(prev),price_as_of:prev?.price_as_of||null,stale:true});console.log('Opinet key missing; keeping last snapshot with delayed refresh status.');process.exit(prev?0:1)}
...
}catch(error){const prev=current();const stale=true;if(prev){prev.stale=stale;fs.writeFileSync(dataPath,JSON.stringify(prev,null,2)+'\n')}writeStatus({ok:false,status:'fetch_failed',checked_at:now.toISOString(),endpoint,error:'Fuel price request failed',snapshot_exists:Boolean(prev),price_as_of:prev?.price_as_of||null,stale});console.error('Opinet refresh failed; preserved last snapshot.');if(!prev)process.exitCode=1}
