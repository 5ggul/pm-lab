...
const idFor=r=>'kea-merged-'+crypto.createHash('sha1').update(`${norm(r.maker_raw)}|${norm(r.model_raw)}|${r.combined_efficiency}|${r.city_efficiency}|${r.highway_efficiency}|${r.range_km}`).digest('hex').slice(0,18);
...
const rows=(display.rows||[]).map(d=>{const candidates=candidatesFor(d);let merge_status='display_only',energyRow=null;if(candidates.length===1){merge_status='exact_unique';energyRow=candidates[0];unique++}else if(candidates.length>1){merge_status='ambiguous_energy_match';ambiguous++}else displayOnly++;return{
  merged_record_id:idFor(d),maker_raw:d.maker_raw,model_raw:d.model_raw,vehicle_class_raw:d.vehicle_class_raw,type_raw:d.type_raw,
  ...
  merge_status,energy_candidate_count:candidates.length,energy_source_record_id:energyRow?.source_record_id??null,energy_candidate_ids:candidates.length>1?candidates.slice(0,10).map(x=>x.source_record_id):[],display_source_record_id:d.source_record_id,
  sources:[{dataset:'KEA_DISPLAY_EFFICIENCY_20260424',source_url:d.source_url,record_id:d.source_record_id},...(energyRow?[{dataset:'KEA_CAR_01_LIST',source_url:energyRow.source_url,record_id:energyRow.source_record_id}]:[])],publishable:false,review_status:'staging_only'
}});
...
const output={schema_version:1,fetched_at:display.fetched_at,display_source_rows:rows.length,energy_source_rows:(energy.rows||[]).length,exact_unique_energy_matches:unique,ambiguous_energy_matches:ambiguous,display_only_rows:displayOnly,distinct_exact_model_groups:groups.length,policy:'Merged rows are staging only. No URL/public/indexable state is generated without reviewed vehicle-family mapping.',rows,groups};
fs.writeFileSync(outPath,JSON.stringify(output,null,2)+'\n');
fs.writeFileSync(statusPath,JSON.stringify({ok:true,fetched_at:display.fetched_at,display_rows:rows.length,energy_rows:(energy.rows||[]).length,exact_unique_energy_matches:unique,ambiguous_energy_matches:ambiguous,display_only_rows:displayOnly,exact_model_groups:groups.length},null,2)+'\n');
