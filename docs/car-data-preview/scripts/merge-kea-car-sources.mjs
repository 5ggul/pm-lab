...
let unique=0,ambiguous=0,consensusCc=0,displayOnly=0;
const rows=(display.rows||[]).map((d,index)=>{
  const candidates=candidatesFor(d);let merge_status='display_only',energyRow=null,consensusCcValue=null,displacementSource=null;
  if(candidates.length===1){merge_status='exact_unique';energyRow=candidates[0];displacementSource=energyRow.displacement_cc!=null?'exact_unique_energy_row':null;unique++}
  else if(candidates.length>1){consensusCcValue=consensusDisplacement(candidates);if(consensusCcValue!=null){merge_status='exact_multi_same_cc';displacementSource='candidate_consensus';consensusCc++}else{merge_status='ambiguous_energy_match';ambiguous++}}
  else displayOnly++;
...
const output={schema_version:3,fetched_at:display.fetched_at,display_source_rows:rows.length,energy_source_rows:(energy.rows||[]).length,exact_unique_energy_matches:unique,consensus_displacement_matches:consensusCc,ambiguous_energy_matches:ambiguous,display_only_rows:displayOnly,distinct_exact_model_groups:groups.length,policy:'Merged rows preserve every KEA display-source row instance. Multiple exact energy candidates are never arbitrarily selected; when every candidate has the same non-null displacement, only that consensus displacement is inherited and candidate IDs remain recorded. No URL/public/indexable state is generated without reviewed vehicle-family mapping.',rows,groups};
fs.writeFileSync(outPath,JSON.stringify(output,null,2)+'\n');
fs.writeFileSync(statusPath,JSON.stringify({ok:true,schema_version:3,fetched_at:display.fetched_at,display_rows:rows.length,energy_rows:(energy.rows||[]).length,exact_unique_energy_matches:unique,consensus_displacement_matches:consensusCc,ambiguous_energy_matches:ambiguous,display_only_rows:displayOnly,exact_model_groups:groups.length},null,2)+'\n');
console.log(`KEA merged: ${rows.length} rich rows / ${unique} unique energy joins / ${consensusCc} consensus-cc joins / ${ambiguous} ambiguous / ${displayOnly} display-only / ${groups.length} model groups`);
