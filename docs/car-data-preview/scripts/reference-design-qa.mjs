...
try{
 for(const width of [375,390,430,1100,1280,1440]){
  ...
  for(const route of ([1100,1440].includes(width)?['cars/']:routes)){
   ...
   assert.deepEqual(await page.locator('header .db-nav a').allTextContents(),['찾기','비교','순위','리콜'],route);
   assert.equal(await page.locator('footer a').filter({hasText:'계산 도구'}).count(),1,route);
   assert.equal(await page.locator('footer a').filter({hasText:'가이드'}).count(),1,route);
   if(route==='cars/'){
    await page.waitForFunction(()=>document.querySelectorAll('.vehicle-card').length===24);
    const geometry=await page.locator('.vehicle-card').evaluateAll(cards=>cards.map(card=>{const group=card.querySelector('.vehicle-card-actions'),g=group.getBoundingClientRect();return {groupWidth:g.width,links:[...group.querySelectorAll('a,button')].map(a=>({top:a.getBoundingClientRect().top,height:a.getBoundingClientRect().height}))}}));
    for(const g of geometry)if(width<=700)assert(g.links.every(l=>l.height>=44));
   ...
   if(route.startsWith('compare/')&&route!=='compare/'){
    if(width<=700){const cards=await page.locator('article[data-pilot-car],article[data-decision-side]').all();assert.equal(cards.length,2);const boxes=await Promise.all(cards.map(c=>c.boundingBox()));assert(boxes[1].y>=boxes[0].y+boxes[0].height-1,route+' mobile vehicles should stack without overlap');}
