...
for(const file of pages.filter(file=>file.includes(path.sep+'rankings'+path.sep))){const html=fs.readFileSync(file,'utf8');for(const match of html.matchAll(/<article class="rank-row"[^>]*data-calc-id="([^"]+)"[^>]*data-family-id="([^"]+)"[^>]*>([\s\S]*?)<\/article>/g)){const href=match[3].match(/<a href="\.\.\/\.\.\/(cars\/[^"]+)"/)?.[1];if(!href)continue;const scope=scopes.get(match[2]),row=sourceRows.get(match[1]),labels=scope?.generation_labels;if(!scope||Array.isArray(labels)&&labels.length>0&&!labels.includes(row?.generation_label)||href!==scope.path)fail(path.relative(root,file),'ranking links a different model/generation');}}
