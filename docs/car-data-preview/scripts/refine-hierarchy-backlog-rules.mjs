...
const RULES=[
  ...
  {maker:/^(?:MINI|Brilliance Shineray)$/i,canonicalMaker:'Brilliance Shineray',makerId:'maker-fbfeb6744606ad63',name:'이티 밴 미니',targetFamilyId:'family-95297a2c89021776',force:true,re:/^이티4?\s*밴\s*미니$/i},
...
for(const [id,gi] of Object.entries(updatedIndex)){
  ...
  for(const r of g.records||[]){const p=gi.family_id==='hyundai-nexo'?'hydrogen':classify(r);f.pt.set(p,(f.pt.get(p)||0)+1);ge.pt.set(p,(ge.pt.get(p)||0)+1);f.record_count++;ge.record_count++;if(g.source_status==='active'){f.active_record_count++;ge.active_record_count++}else f.archived_record_count++;if(calcReady(r,p)){f.calculator_ready_record_count++;ge.calculator_ready_record_count++}}
}
