...
const powertrainLabel={gasoline:'가솔린',diesel:'디젤',lpg:'LPG',hybrid:'하이브리드',phev:'플러그인 하이브리드',electric:'전기',hydrogen:'수소',unknown:''};
...
for(const [slug,maker] of makers){
  ...
  const cards=rows.map(family=>{
    ...
    const action=family.static_detail_path?`<a class="maker-model-link" href="${esc(href)}">연비·세금·연료·충전비 <span aria-hidden="true">→</span></a>`:`<a class="maker-model-link" href="${esc(href)}">신고 사양 ${family.active_record_count.toLocaleString('ko-KR')}개 <span aria-hidden="true">→</span></a>`;
