...
for(const [slug,maker] of makers){
  ...
  const cards=rows.map(family=>{
    ...
    const unit=main?.powertrain==='electric'?'km/kWh':main?.powertrain==='hydrogen'?'km/kg':'km/L';
    const efficiency=main?`${range(main.combined_efficiency)} ${unit}`:'공개 연비 없음';
    ...
    return `<article class="maker-model">${picture(family)}<div class="maker-model-copy"><p>${esc(trains||'연료 정보 없음')}</p>${title}<dl><div><dt>공개 사양</dt><dd>${family.active_record_count.toLocaleString('ko-KR')}개</dd></div><div><dt>복합 효율</dt><dd>${esc(efficiency)}</dd></div></dl>${action}</div></article>`;
  }).join('');
