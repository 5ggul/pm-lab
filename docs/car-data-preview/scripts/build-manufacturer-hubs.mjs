...
const efficiencyRows=family=>(family.powertrains||[]).filter(item=>item.combined_efficiency&&item.powertrain!=='unknown').map(item=>{
  const label=powertrainLabel[item.powertrain],unit=item.powertrain==='electric'?'km/kWh':item.powertrain==='hydrogen'?'km/kg':'km/L';
...
for(const [slug,maker] of makers){
  ...
  const itemList={'@context':'https://schema.org','@type':'ItemList',name:`${maker} 차량 연비·자동차세`,numberOfItems:linkedRows.length,itemListElement:linkedRows.map((family,index)=>({'@type':'ListItem',position:index+1,name:`${maker} ${family.family_name}`,url:pageUrl(family.static_detail_path)}))};
