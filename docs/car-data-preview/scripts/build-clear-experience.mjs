...
function image(id){const r=reviewedImage(id),car=catalog.cars.find(c=>c.id===id);return `<img class="pilot-photo" src="${esc(r.url)}" alt="${esc(car.maker)} ${esc(car.model)} 대표 차량 사진" width="${r.width}" height="${r.height}" loading="lazy"><details class="image-credit"><summary>사진 출처</summary><p>${esc(r.generation)} · <a href="${esc(r.source_page)}">${esc(r.author)}</a> · <a href="${esc(r.license_url)}">${esc(r.license)}</a></p></details>`;}
...
for(const type of rankTypes){
  ...
  const scopeDate=type.metric==='energy-cost'?`유가 ${fuel.price_as_of} · 목록 생성 ${date}`:`자료 기준 ${date}`;
