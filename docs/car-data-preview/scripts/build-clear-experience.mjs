...
const staticRecords=read('data/static-model-pages.json').records;
const staticPathByFamily=new Map(staticRecords.map(r=>[r.family_id,r.path]));
const staticScopeByFamily=new Map(staticRecords.map(r=>[r.family_id,r]));
...
function image(id){const r=reviewedImage(id),car=catalog.cars.find(c=>c.id===id),spec=r.depicted_spec?`사진 속 사양: ${esc(r.depicted_spec)} · 표시 수치와 별개<br>`:'';return `<img class="pilot-photo" src="${esc(r.url)}" alt="${esc(car.maker)} ${esc(car.model)} 대표 차량 사진" width="${r.width}" height="${r.height}" loading="lazy">${spec?`<p class="home-photo-spec">${spec.replace('<br>','')}</p>`:''}<details class="image-credit"><summary>사진 출처</summary><p>${esc(r.generation)} · ${spec}<a href="${esc(r.source_page)}">${esc(r.author)}</a> · <a href="${esc(r.license_url)}">${esc(r.license)}</a></p></details>`;}
...
const recallData=read('data/recalls.json'),notices=recallData.notices.slice().sort((a,b)=>b.date.localeCompare(a.date)).slice(0,3);
...
fs.writeFileSync(path.join(root,'index.html'),home.replace('<h2>최근 리콜</h2><a class="section-link" href="./recalls/">전체 공지 →</a>','<h2>수록 리콜 공지</h2><a class="section-link" href="./recalls/">수록 공지 →</a>').replace('<div class="recall-list" id="homeRecalls">',`<p class="recall-coverage">${notices.length}건 표시 · 자료 확인 ${recallData.data_as_of} · 전체·최신 리콜 목록은 <a href="https://www.car.go.kr/ri/stat/list.do">자동차리콜센터</a>에서 확인</p><div class="recall-list" id="homeRecalls">`));
...
for(const type of rankTypes){
  ...
  const rows=selected.map((r,i)=>{const value=rankingValue(r,type);if(value!==lastValue)rank=i+1;lastValue=value;const shown=type.unit==='원'?Math.round(value).toLocaleString('ko-KR'):value;const scope=staticScopeByFamily.get(r.family_id),detailPath=scope?.generation_labels?.includes(r.generation_label)?scope.path:null;const action=detailPath?`<a href="../../${esc(detailPath)}">${esc(r.family_name)} 상세 사양 →</a>`:'<span class="rank-detail-unavailable">정적 상세에 없는 신고 사양</span>';return `<article class="rank-row" data-rank="${rank}" data-calc-id="${esc(r.calc_id)}" data-family-id="${esc(r.family_id)}" data-metric-value="${value}" data-metric-direction="${type.direction}"><span class="rank-position">${rank}</span><div><h2>${esc(r.maker)} ${esc(r.family_name)}</h2><p title="공식 원문: ${esc(r.raw_model)}">${esc(displayVariant(r))}</p>${action}</div><div class="rank-value">${shown} <small>${type.unit}</small></div></article>`;}).join('');
  ...
  const basis=type.metric==='efficiency'
    ...
    :type.metric==='energy-cost'
      ?`연 20,000km ÷ 복합연비 × ${fuel.source} ${fuel.price_as_of} 단가로 계산했습니다. 전기차는 충전 요금이 없어 제외했습니다.`
  ...
  const scopeDate=type.metric==='energy-cost'?`유가 ${fuel.price_as_of} · 공식 사양 ${date}`:`공식 사양 ${date}`;
  const priceLine=type.metric==='energy-cost'?`<p class="rank-scope" data-ranking-prices>${[['휘발유','gasoline'],['경유','diesel'],['LPG','lpg']].map(([name,key])=>`${name} ${Number(fuel.prices[key]).toLocaleString('ko-KR',{minimumFractionDigits:2})}원/L`).join(' · ')}</p>`:'';
