...
function rowMarkup(r){return `<tr><th scope="row">${esc(r.raw_model)}</th><td>${fuelName[r.powertrain]||esc(r.powertrain)}</td><td>${r.combined_efficiency} ${unit(r.powertrain)}</td><td>${r.city_efficiency??'—'}</td><td>${r.highway_efficiency??'—'}</td><td>${r.range_km?Number(r.range_km).toLocaleString('ko-KR')+'km':'—'}</td><td>${taxText(r)}</td><td>${costText(r)}</td></tr>`}
...
<section class="dossier-band"><div class="dossier-shell"><h2>숫자를 읽는 기준</h2><p>${esc(m.reading)}</p></div></section>
<section class="dossier-section"><div class="dossier-shell"><div class="dossier-heading"><h2>사양 범위</h2><p>막대는 동력 방식별 최고 복합연비 또는 전비의 위치를 보여 줍니다. 숫자에는 등록 사양 범위를 함께 표시합니다. km/L와 km/kWh는 단위가 달라 서로 길이를 비교하지 않습니다.</p></div><div class="dossier-meters">${meterRows(sourceRows)}</div></div></section>
<section class="dossier-section"><div class="dossier-shell"><div class="dossier-heading"><h2>대표 사양</h2><p>공식 등록 행 가운데 연비 범위와 세금 계산 가능 여부가 드러나는 사양을 추렸습니다. 값이 없는 칸은 다른 사양에서 가져와 채우지 않았습니다.</p></div><div class="dossier-table-wrap" tabindex="0" role="region" aria-label="${esc(m.model)} 사양별 비용 표"><table class="dossier-table"><thead><tr><th>공식 등록 사양</th><th>동력</th><th>복합</th><th>도심</th><th>고속</th><th>1회 주행</th><th>연 자동차세</th><th>2만km 에너지비</th></tr></thead><tbody>${rows.map(rowMarkup).join('')}</tbody></table></div><p class="dossier-note">자동차세는 비영업용 승용 신차 정상세액과 지방교육세 30%를 합산했습니다. 연납·차령 경감·개별 감면은 제외합니다. 전기차 충전비는 이용 요금 차이가 커 기본값을 만들지 않습니다.</p></div></section>
<section class="dossier-section"><div class="dossier-shell"><div class="dossier-heading"><h2>확인할 숫자</h2></div><div class="dossier-facts"><article><b>01</b><h3>${electricOnly?'복합전비':'복합연비·전비'}</h3><p>${fmtRange(eff,'')} ${electricOnly?'km/kWh':'(동력별 단위 확인)'}. 휠·구동·세부 사양이 다르면 같은 모델도 값이 달라집니다.</p></article><article><b>02</b><h3>연 자동차세</h3><p>${taxes[0]==null?'배기량이 연결된 행에서만 표시합니다.':moneyRange(taxes)}. 실제 고지액은 등록 시점과 차령에 따라 달라질 수 있습니다.</p></article><article><b>03</b><h3>연 20,000km</h3><p>내연기관은 거리 ÷ 복합연비 × 오피넷 단가로 계산합니다. 전기차는 실제 충전단가를 입력해 계산합니다.</p></article></div><div class="dossier-actions"><a href="${prefix}tools/annual-cost/?fa=${encodeURIComponent(m.family_id)}">내 조건으로 비용 계산</a><a href="${prefix}cars/family/?id=${encodeURIComponent(m.family_id)}">전체 연식·사양 보기</a><a href="${prefix}compare/?fa=${encodeURIComponent(m.family_id)}">다른 차량과 비교</a></div></div></section>
<section class="dossier-section"><div class="dossier-shell"><div class="dossier-heading"><h2>출처와 범위</h2><p>연비·전비와 배기량은 한국에너지공단 자동차 표시연비·에너지효율 공개 자료를 사용했습니다. 사진은 라이선스가 확인된 대표 이미지입니다.</p></div><div class="dossier-actions"><a href="${prefix}data-sources/">공식 데이터 출처</a><a href="${esc(calc.tax.source)}">자동차세 법령</a><a href="${prefix}methodology/">계산 기준</a><a href="${prefix}media-policy/">사진 이용안내</a></div><p class="dossier-note">자료 생성 ${esc(calc.generated_at.slice(0,10))} · 유가 ${esc(calc.fuel_price.price_as_of)} · 이 페이지는 ${m.generation_labels.map(esc).join(', ')} 등록 사양을 다룹니다. 보험·정비·구매가격·감가상각은 포함하지 않습니다.</p></div></section></main><footer class="db-footer"><div class="db-shell"><strong>내차데이터</strong><p>자동차세와 연료·충전비를 같은 조건으로 비교합니다.</p></div></footer></body></html>`;
 write(m.path,html);
}
