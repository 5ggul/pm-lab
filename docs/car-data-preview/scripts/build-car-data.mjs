...
const sources=manifest.vehicles.map(entry=>{const data=readJson(path.join(vehicleRoot,entry.file));if(Boolean(data.indexable)!==Boolean(entry.indexable)) throw new Error(`${entry.id}: manifest indexable mismatch`);return {...data,image:reviewedImage(entry.id),display_name:entry.display_name};});
...
const photoCredit=c=>`<details class="photo-credit"><summary>사진 출처</summary><span>${esc(c.imageMeta.author)} · <a href="${esc(c.imageMeta.sourceUrl)}" target="_blank" rel="noopener">원본</a> · <a href="${esc(c.imageMeta.licenseUrl)}" target="_blank" rel="noopener">${esc(c.imageMeta.license)}</a> · 사진: ${esc(c.imageMeta.generation)} · 연식·트림에 따라 외관이 다를 수 있습니다.</span></details>`;
const carCard=c=>`<a class="car-card" href="${esc(c.path)}"><div class="car-card-media"><img loading="lazy" src="${esc(c.image)}" alt="${esc(c.model)}" width="900" height="600"></div>${photoCredit(c)}<div class="car-card-body"><div class="car-meta">${esc(c.maker)} · ${esc(c.segment)} · ${esc(c.yearLabel)}</div><h3>${esc(c.model)}</h3><div class="car-spec">${esc(c.rep.label)}</div><div class="car-numbers"><div class="car-number-row"><span>${c.energy==='ev'?'복합전비':'복합연비'}</span><strong>${c.rep.combined} ${c.energy==='ev'?'km/kWh':'km/L'}</strong></div><div class="car-number-row"><span>연간 정상 자동차세</span><strong>${fmt(c.rep.tax)}원</strong></div>${c.energy==='ev'?`<div class="car-number-row"><span>1회 충전거리</span><strong>${fmt(c.rep.range)} km</strong></div>`:`<div class="car-number-row"><span>20,000km 연료·충전비</span><strong>${fmt(c.rep.annualEnergy)}원</strong></div>`}</div><div class="car-arrow">사양별 상세 →</div></div></a>`;
const costRows=`<div class="cost-row head"><div>차량 / 기준 사양</div><div>효율</div><div>자동차세</div><div>연료·충전비</div><div>합계</div></div>`+publicCars.map(c=>`<div class="cost-row"><div><div class="model">${esc(c.model)}</div><small>${esc(c.rep.label)}</small></div><div><small class="mobile-label">효율</small><strong>${c.rep.combined} ${c.energy==='ev'?'km/kWh':'km/L'}</strong></div><div><small class="mobile-label">자동차세</small><span>${fmt(c.rep.tax)}원</span></div><div><small class="mobile-label">연료·충전비</small><span>${c.energy==='ev'?'충전단가 입력':fmt(c.rep.annualEnergy)+'원'}</span></div><a class="home-total" href="${c.energy==='ev'?'./tools/annual-cost/?car='+c.id:c.path}">${c.energy==='ev'?'계산 →':fmt(c.rep.total)+'원 →'}</a></div>`).join('');
...
function modelSchema(car){const url=new URL(car.path.slice(2),siteConfig.baseUrl).href;return `<script type="application/ld+json" data-build-schema>{"@context":"https://schema.org","@graph":[{"@type":"WebPage","name":"${esc(car.model)} 공식 사양·자동차세·연료·충전비","url":"${url}","dateModified":"${car.reviewedOn.replaceAll('.','-')}","mainEntity":{"@id":"${url}#vehicle"}},{"@type":"Vehicle","@id":"${url}#vehicle","name":"${esc(car.maker+' '+car.model)}","manufacturer":{"@type":"Organization","name":"${esc(car.maker)}"},"model":"${esc(car.model)}","bodyType":"${esc(car.segment)}"},{"@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"홈","item":"${siteConfig.baseUrl}"},{"@type":"ListItem","position":2,"name":"${esc(car.maker)}"},{"@type":"ListItem","position":3,"name":"${esc(car.model)}","item":"${url}"}]}]}</script>`;}
function annotateMetrics(html){const map=[['복합연비','efficiency'],['복합전비','efficiency'],['연간 정상 자동차세','tax'],['연간 자동차세','tax'],['세금+유류비','annual-total'],['세금+연료·충전비','annual-total'],['1회 충전거리','range']];for(const [labelText,field] of map){const re=new RegExp(`<small>${labelText}<\\/small><b(?![^>]*data-field)`,`g`);html=html.replace(re,`<small>${labelText}</small><b data-field="${field}"`);}return html;}
