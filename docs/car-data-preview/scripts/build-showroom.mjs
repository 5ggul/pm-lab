...
const hero=`<section class="editorial-hero" aria-label="차량 검색과 비교"><div class="hero-intro"><div><h1>차량별 연비·전비·자동차세·연료비 비교</h1></div><form class="db-search" action="./cars/" method="get"><input name="q" type="search" placeholder="어떤 차가 궁금하세요?" aria-label="차종 또는 제조사"><button type="submit">검색 <span aria-hidden="true">↗</span></button></form></div><figure class="hero-photograph"><picture><source type="image/webp" srcset="${image.files.map(f=>'./'+f.path+' '+f.width+'w').join(', ')}" sizes="100vw"><img class="pilot-photo" src="./${image.files.at(-1).path}" width="${image.original_width}" height="${image.original_height}" alt="사람 없이 스튜디오에 주차된 검은색 아이오닉 6의 측면" fetchpriority="high" decoding="async"></picture></figure><nav class="hero-utility-links" aria-label="주요 기능">${[['차량 찾기','cars/'],['세금·연료·충전비 계산','tools/annual-cost/'],['차량 비교','compare/'],['자동차 순위','rankings/']].map(([label,url])=>`<a href="./${url}">${label}</a>`).join('')}</nav></section>`;
const heroWithNote=hero.replace('</figure>','</figure><p class="hero-photo-note">2023년 촬영한 아이오닉 6 사진 · 표시 사양과 별개 · <a href="./media-policy/#home-hero-photo">사진 출처</a></p>');
if(start>=0){
 ...
 html=html.slice(0,start)+heroWithNote+catalog+html.slice(end);
 html=html.replace(/<script id="studio-data"[\s\S]*?<\/script><script type="module" src="[^\"]*assets\/studio.js(?:\?[^\"]*)?"><\/script>/,'');
 html=html.replace(/<body class="/,'<body class="showroom-home ');
}else if(!html.includes('class="editorial-hero"'))throw Error('Generate studio before editorial home');
if(!html.includes('assets/showroom-ui.css'))html=html.replace('</head>','<link rel="stylesheet" href="./assets/showroom-ui.css"></head>');
