...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');if(!/GENERATED:HOME-CATALOG:START[\s\S]+class="car-card" href=/i.test(home))fail('home: static car links missing');for(const c of catalog.cars.filter(c=>c.indexable))if(!home.includes(String(c.rep.combined)))fail(`home: ${c.id} static efficiency missing`);
