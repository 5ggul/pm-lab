...
const home=fs.readFileSync(path.join(root,'index.html'),'utf8');if(!/GENERATED:HOME-CATALOG:START[\s\S]+<a class="car-card" href=/i.test(home))fail('home: static car links missing');for(const c of catalog.cars.filter(c=>c.indexable))if(!home.includes(String(c.rep.combined)))fail(`home: ${c.id} static efficiency missing`);
...
// Fixed-price fixtures keep numerical regression checks stable across daily fuel refreshes.
const regressionPrices={gas:1859.66,diesel:1843.8,lpg:1098.59};
for(const [field,key] of [['gasPrice','gasoline'],['dieselPrice','diesel'],['lpgPrice','lpg']])if(catalog[field]!==fuelPrice.prices[key])fail(`catalog ${field}: does not match fuel snapshot`);
for(const c of catalog.cars){const p=c.rep.fuelType==='diesel'?catalog.dieselPrice:c.rep.fuelType==='lpg'?catalog.lpgPrice:c.energy==='ev'?null:catalog.gasPrice;const expected=p==null?null:Math.round(catalog.annualKm/c.rep.combined*p);if(c.rep.annualEnergy!==expected)fail(`${c.id}: generated energy cost does not match current price`);}
function regression(carId,variantId,expected){const c=catalog.cars.find(x=>x.id===carId),v=c?.variants.find(x=>x.id===variantId);if(!c||!v){fail(`regression ${carId}/${variantId}: missing`);return}const tax=v.fuel==='electric'?130000:ccTax(v.cc),price=v.fuel==='diesel'?regressionPrices.diesel:v.fuel==='lpg'?regressionPrices.lpg:v.fuel==='electric'?null:regressionPrices.gas,e=price==null?null:energy(20000,v.combined,price),total=e==null?null:tax+e;for(const [k,a,b] of [['efficiency',v.combined,expected.efficiency],['tax',tax,expected.tax],['energy',e,expected.energy],['total',total,expected.total]])if(a!==b)fail(`regression ${carId}/${variantId}: ${k} ${a} != ${b}`)}
