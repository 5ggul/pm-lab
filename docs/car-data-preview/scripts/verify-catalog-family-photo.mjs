...
try{
  ...
  const matching=page.locator('.vehicle-card').filter({hasText:expectedText});
  if(await matching.count()!==1)throw new Error(`Expected one ${expectedText} card`);
  const photo=await matching.locator('img').evaluate(img=>({src:img.currentSrc||img.src,complete:img.complete,naturalWidth:img.naturalWidth}));
  if(!photo.complete||photo.naturalWidth<300||!photo.src.includes(familyId))throw new Error(`Invalid photo ${JSON.stringify(photo)}`);
