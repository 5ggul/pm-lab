...
function holdReason(rel){
 ...
 if(/^cars\/[^/]+\/index\.html$/.test(rel))return rel==='cars/models/index.html'?'Overlapping legacy model directory':'Manufacturer directory lacks independent editorial value';
 if(/^cars\/[^/]+\/[^/]+\/.+\/index\.html$/.test(rel))return 'Legacy specification subset overlaps its model page';
 if(rel==='compare/dimensions/index.html')return 'Retired size-comparison redirect';
 return null;
}
...
for(const file of pages){
 ...
 reports.push({path:'/'+rel.replace(/index\.html$/,''),kind,status:hold?'hold':failures.length?'failed':'release_ready',reason:hold||undefined,title,h1,visible_characters:visible.length,checks,failures});
}
...
const byKind=Object.fromEntries([...new Set(reports.map(p=>p.kind))].sort().map(kind=>[kind,{total:reports.filter(p=>p.kind===kind).length,release_ready:reports.filter(p=>p.kind===kind&&p.status==='release_ready').length,hold:reports.filter(p=>p.kind===kind&&p.status==='hold').length,failed:reports.filter(p=>p.kind===kind&&p.status==='failed').length}]));
const report={schema_version:1,generated_at:new Date().toISOString(),preview_noindex_unchanged:siteConfig.indexingEnabled===false,production_switch:false,production_origin:null,summary:{total:reports.length,release_ready:ready.length,hold:reports.filter(p=>p.status==='hold').length,failed:failed.length,by_kind:byKind},duplicates:{titles:duplicateTitles,h1:duplicateH1},pages:reports.sort((a,b)=>a.path.localeCompare(b.path))};
