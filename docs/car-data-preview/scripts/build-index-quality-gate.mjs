...
for(const file of pages){
 ...
 if(!hold){
  ...
  if(kind==='catalog')checks.push(check('static_vehicle_rows',count(html,/<li><div><span>/g)>=24,count(html,/<li><div><span>/g)),check('search_enhancement',/assets\/catalog-consumer\.js/.test(html)),check('photo_index',visible.includes(`대표 사진 ${photoCount}종`)));
  ...
  if(kind==='vehicle')checks.push(check('vehicle_photo',/<(?:picture|img)\b/i.test(html)),check('structured_specification',/<(?:table|dl)\b/i.test(html)||/class="(?:metrics-strip|data-grid|spec-grid|spec-card|model-metrics)"/.test(html)),check('efficiency',/복합\s*(?:연비|전비)/.test(visible)),check('tax',/자동차세/.test(visible)),check('cost',/(?:연료비|충전비|연료·충전비)/.test(visible)),check('official_source',/(?:한국에너지공단|제조사 자료|공식 연비|공식 전비)/.test(visible)),check('vehicle_schema',hasSchema(html,'Vehicle')));
