...
for(const [i,f] of small.families.entries())for(const [key,value] of Object.entries(f)){const expected=key==='manufacturer_detail'?Boolean(full.families[i][key]):key==='record_count'?(full.families[i].active_record_count||0):key==='powertrains'?(full.families[i].powertrains||[]).map(p=>({powertrain:p.powertrain,combined_efficiency:p.combined_efficiency})):key==='path'?(full.families[i].static_detail_path||null):full.families[i][key];assert.deepEqual(value,JSON.parse(JSON.stringify(expected)),`${f.family_id} ${key}`)}
