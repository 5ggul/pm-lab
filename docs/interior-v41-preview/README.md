...
- production-shell read isolation masks `interior-quote-v5`, `interior-compare-v5`, and `interior-compare-v6` only during app-v21 initialization, then restores the original `Storage.prototype.getItem` at DOMContentLoaded. Review-only keys still pass through. VM read-mask regression: 10/10 PASS.
- robustness probe now also verifies the read-mask in an isolated browser `srcdoc` realm without writing real production-named storage: protected reads are null before DOMContentLoaded, review reads pass through, and the underlying reader is restored afterward.
