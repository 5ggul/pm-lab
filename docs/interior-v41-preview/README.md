...
- Hosted probes prepared: `self-check.html` 58 checks, `failure-probe.html` 8 checks, `writer-concurrency-probe.html` 7 checks, `pending-recovery-probe.html` 9 checks, `stale-transfer-probe.html` 9 checks, `robustness-probe.html` 18 checks. Total: 109 hosted automatic checks. None is reported as PASS until an external HTTPS preview exists.
