...
공통 quote-check writer는 새 전송 전에 `currentPendingState()`를 확인합니다.
...
기본 compare cleanup과 새 writer가 가능한 순서로 끼어들도록 시뮬레이션했습니다.
...
기본 `quote-compare/`에 별도 Web Lock cleanup 코드를 중복 추가할 필요는 없습니다.

- production-shell: compare cleanup 자체도 writer lock에 참여
- 기본 harness: writer가 fresh partial까지 점유로 간주해 cleanup 중간 진입을 차단
- 두 경로 모두 newer transfer 삭제 방지와 cleanup-window write 방지 조건을 만족

기본 하네스 코드를 불필요하게 재작성하지 않고, 공통 writer의 partial blocker를 단일 안전 규칙으로 유지합니다.
