(() => {
  ...
  function injectStyles(){
    ...
    style.textContent=`
      .v42-handoff-wrap{display:inline-flex;align-items:center}
      .v42-handoff-button{font-weight:800}
      ...
      .v42-handoff-dialog::backdrop{background:rgba(23,26,24,.48)}
      .v42-handoff-dialog [data-v42-dialog-body]{padding:22px}
      .v42-handoff-dialog h2{margin:0 0 8px;font-size:20px;letter-spacing:-.03em}
      .v42-handoff-dialog p{margin:0 0 16px;color:var(--muted,#777168);font-size:13px;line-height:1.6}
      .v42-targets{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;border:0;padding:0;margin:0 0 18px}
      .v42-targets label{border:1px solid var(--line,#C9C4B8);padding:11px 8px;text-align:center;cursor:pointer;font-weight:700}
      .v42-targets input{margin-right:5px}
      .v42-dialog-actions{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap}
      .v42-handoff-summary{border-top:1px solid var(--line,#C9C4B8);border-bottom:1px solid var(--line,#C9C4B8);padding:12px 0;margin:14px 0 18px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px}
      .v42-handoff-summary span{display:block;color:var(--muted,#777168);font-size:12px}
      .v42-handoff-summary strong{display:block;margin-top:3px;font-size:16px}
  ...
  function initQuoteCheck(){
    ...
    wrap.className='v42-handoff-wrap';
    const button=document.createElement('button');
    button.type='button';
    button.className='v42-handoff-button';
    button.dataset.v42SendToCompare='';
    button.textContent='비교표로 보내기';
    wrap.append(button);
    actions.prepend(wrap);

    const dialog=makeDialog();
    const body=$('[data-v42-dialog-body]',dialog);
    ...
    $('[data-v42-send]',dialog).addEventListener('click',()=>{
      try{
        ...
        location.assign(`${BASE}/quote-compare/`);
      }catch(error){
        dialog.close();
        alert(`비교표로 보내지 못했습니다. ${error.message}`);
      }
    });
  }

  function isFreshHandoff(handoff){
    if(!handoff||!VENDORS.includes(handoff.target)) return false;
    const created=Date.parse(handoff.createdAt||'');
    return Number.isFinite(created)&&Date.now()-created>=0&&Date.now()-created<=HANDOFF_MAX_AGE_MS;
  }

  function compareKey(el){
  ...
  function readCompareDOM(){
    const out={};
    $$('[data-compare-table] [data-vendor]').forEach(el=>out[compareKey(el)]=el.value);
    return out;
  }

  function saveCompareDOM(){
    const data=readCompareDOM();
    return COMPARE_KEYS.every(key=>setJSON(key,data));
  }
  ...
  function applyQuoteToVendor(quote,target){
    for(const id of ITEMS){
      ...
      const state=$(`[data-vendor="${target}"][data-state]`,row);
      const amount=$(`[data-vendor="${target}"][data-amount]`,row);
      if(state) state.value=['included','separate','missing'].includes(item.state)?item.state:'missing';
      if(amount) amount.value=safeText(item.amount);
      for(const el of [state,amount]){
        if(!el) continue;
  ...
  function quoteSummary(quote){
    let entered=0,total=0,missing=0;
    for(const id of ITEMS){
      const item=quote.items?.[id]||{};
      if(item.state!=='missing'||safeText(item.amount).trim()) entered++;
      if(item.state==='missing') missing++;
      total+=Number(item.amount||0);
    }
    return {entered,total,missing};
  }
  ...
  function initQuoteCompare(){
    ...
    let handoff=getJSON(HANDOFF_KEY,null);
    ...
    $('[data-v42-import-cancel]',dialog).addEventListener('click',()=>{
      del(HANDOFF_KEY);
      dialog.close();
      dialog.remove();
    });

    $('[data-v42-import-apply]',dialog).addEventListener('click',()=>{
      if(!applyQuoteToVendor(quote,target)){
        alert('비교표 저장에 실패했습니다. 브라우저 저장소를 확인해 주세요.');
        return;
      }
      del(HANDOFF_KEY);
      dialog.close();
      dialog.remove();
      const total=$(`[data-total="${target}"]`);
  ...
  window.InteriorQuoteHandoff42={readQuote,readCompareDOM,applyQuoteToVendor,QUOTE_KEY,HANDOFF_KEY,COMPARE_KEYS};
