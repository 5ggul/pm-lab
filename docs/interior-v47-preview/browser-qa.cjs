...
async function runDesktop(browser,report){
  ...
  assert.equal(model.vendors.length,3);assert.ok(model.vendors.reduce((s,v)=>s+v.total,0)>2);assert.ok(model.vendors.reduce((s,v)=>s+v.completed,0)>=2);assert.match(model.text,/전체 확인 완료/);assert.match(model.text,/철거 범위와 폐기물 반출 포함이라고 답변/);assert.match(model.text,/아직 확인할 질문/);assert.doesNotMatch(model.text,/추천 업체|가장 저렴|적정가격|계약 적합|법률 판단/);
