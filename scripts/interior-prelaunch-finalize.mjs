...
sitemap=sitemap.replace(/\s*<url><loc>https:\/\/5ggul\.github\.io\/pm-lab\/interior-cost-preview\/region\/<\/loc><lastmod>[^<]+<\/lastmod><\/url>\s*/g,'\n');
...
const allowlist={
  ...
  hold_rules:[
    {pattern:'/region/',reason:'지역 검수 표본 공개 전에는 지역 허브도 production surface 보호 상태 유지. 소유자 최종 승인 시 별도 해제.'},
    {pattern:'/region/{sido}/',reason:'지역 검수 표본 N<20. 지역 상세는 계속 noindex 유지.'},
...
const readiness={
  ...
  completed:{
    ...
    'P1-5':'region N=0 repetition removed; all region surfaces remain protected from production sitemap until owner final approval',
  ...
  intentionally_blocked:[
    ...
    'Release /region/ hub from protected production surface after owner approval',
...
if(allowlist.index_routes.includes('/region/'))throw new Error('Protected region hub leaked into index allowlist');
