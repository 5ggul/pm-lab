...
- 수집기는 가격·기간·지역·조건 같은 사실만 `copyContext`로 넘깁니다.
- 발행 문장은 고정 템플릿이 아니라 정보 블록 조합으로 여러 구조를 만든 뒤 선택합니다.
- 최근 게시글의 제목 구조, 본문 skeleton, 첫 문장, 마지막 문장, 문장 길이 패턴을 저장해 반복을 피합니다.
- AI Tone Score 30 이상, 가짜 구매/사용 경험, 애교 어미 3회 이상은 발행 전 차단합니다.
- 같은 판매처나 같은 주제가 연속되면 후보 우선순위를 낮춥니다.
- 좋은 후보가 없으면 일일 목표 수를 억지로 채우지 않습니다.
- 해외통화 가격을 원화로 오인한 핫딜, 일부만 무료인 행사를 전체 무료로 표현하는 후보는 수집 단계에서 제외합니다.
