...
const dataset=buildDataset({rows,sourceTotalCount:3,rawRowCount:3,validRowCount:3,minPublishedRows:2,start:'20250101',end:'20250130',windowDays:30,windowCount:1,nonemptyWindowCount:1,generatedAt:'2026-02-02T00:00:00.000Z'});
const range={start:'20250101',end:'20250130',days:29};
const collection={sourceTotalCount:3,rawRowCount:3,validRowCount:3,windowDays:30,windowCount:1,nonemptyWindowCount:1};
...
for(const [message,code] of [
  ...
  ['no valid PPS G2B unit price rows for 20250101-20250130 across 1 bounded windows','no_rows_in_query_windows']
...
for(const required of ["mode:'read_only_preflight'",'repository_write:false','production_deploy:false','PREFLIGHT_OUT_DIR','PREFLIGHT_STRICT','minPublishedRows:1','probeGateway','classifyCollectionError','service_key_not_registered','no_rows_in_query_windows','http_probe_service_key_sent:false'])if(!source.includes(required))errors.push(`source-contract:${required}`);
if(errors.length){console.error(JSON.stringify({ok:false,errors},null,2));process.exit(1)}
console.log(JSON.stringify({ok:true,synthetic_only:true,source:'15129415',report_schema:'1.2',ready_case:true,blocked_floor_case:true,stale_case:true,bounded_window_case:true,classified_external_blockers:true,credential_free_gateway_probe:true,http_probe_service_key_sent:false,repository_write:false,git_commit:false,git_push:false,production_deploy:false,artifact_report:'public-price-preflight.json'},null,2));
