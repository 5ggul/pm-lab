...
const errors=[];
const required=[
  'index.html','assets/site-v6.css','assets/mobile-v6.css','assets/app-v6.js','assets/search-v6.js','assets/cost-v6.js','assets/cost-v6.css','assets/guide-v6.css','assets/compare-diagnostics-v6.js','assets/compare-diagnostics-v6.css','data/index.html','data/data-contract.json','data/construction-cost-index.json','data/quote-statistics.json',
  'quote-compare/index.html','calculator/index.html','checklist/index.html','work-match/index.html','data-status/index.html','reference-prices/index.html','statistics/index.html','interior-cost/index.html','interior-cost/24-pyeong/index.html','interior-cost/30-pyeong/index.html','interior-cost/32-pyeong/index.html','interior-cost/34-pyeong/index.html','interior-cost/40-pyeong/index.html',
...
for(const text of ['인테리어 비용·견적 검수','총액 비교 전에 5단계','work-match/','quote-check/','quote-compare/','checklist/','data/','데이터 모음','data-month-change','data-year-change','data/construction-cost-index/','guides/','compare-diagnostics-v6.js','assets/search-v6.js']) if(!home.includes(text)) errors.push(`home-required:${text}`);
for(const step of ['공종 매칭','누락·별도금액·1식·사양','A·B·C 업체','계약 체크리스트','원본 JSON'])if(!home.includes(step))errors.push(`home-workflow:${step}`);
const dataHub=fs.readFileSync(path.join(root,'data/index.html'),'utf8');
for(const token of ['../data-status/','data-contract.json','work-match-rules.json','public-price-sources.json'])if(!dataHub.includes(token))errors.push(`data-hub-link:${token}`);
...
if(contract.version!=='6.3-review'||contract.production_quality?.orphan_indexable_pages_allowed!==0||contract.production_quality?.minimum_indexable_text_length!==220)errors.push('production-contract');
if(contract.production_quality?.release_manifest?.required!==true||contract.production_quality?.release_manifest?.hash_algorithm!=='sha256')errors.push('release-manifest-contract');
if(contract.release_policy?.preview_first!==true||contract.release_policy?.explicit_owner_approval_required!==true||contract.release_policy?.technical_validation_required!==true||contract.release_policy?.production_deploy_action!=='이 검수 workflow는 production 배포를 수행하지 않고 후보 artifact만 생성')errors.push('release-policy-contract');
for(const mode of ['blocked_technical','blocked_manual_approval','core_only','full_data'])if(!contract.release_policy?.release_modes?.[mode])errors.push(`release-mode:${mode}`);
if(contract.public_price_policy?.live_preflight?.mode!=='artifact_only'||contract.public_price_policy?.live_preflight?.repository_write!==false||contract.public_price_policy?.live_preflight?.production_deploy!==false)errors.push('public-price-preflight-contract');
if(contract.release_policy?.preflight_workflows?.public_price_api?.artifact_only!==true||contract.release_policy?.preflight_workflows?.public_price_api?.mutates_repository!==false||contract.release_policy?.preflight_workflows?.production_domain?.artifact_only!==true||contract.release_policy?.preflight_workflows?.production_domain?.deploys!==false)errors.push('release-preflight-contract');
if(contract.production_quality?.production_base_url?.https_required!==true||contract.production_quality?.production_base_url?.github_pages_preview_forbidden_by_default!==true)errors.push('production-base-url-contract');
...
for(const token of ['management-fee/','change-order/','partial-vs-full/','self-vs-turnkey/','kitchen-quote/','wallpaper-flooring/','waste-separate/','bathroom-one-set/','data/','data-status/','checklist/','work-match/']) if(!search.includes(token)) errors.push(`search-route:${token}`);
...
console.log(JSON.stringify({ok:true,html_count:htmlFiles.length,home_workflow_steps:5,quote_sample_count:quote.sample_count,quote_public_threshold:quote.minimum_public_sample,data_types:Object.keys(contract.types),data_contract:contract.version,release_modes:Object.keys(contract.release_policy.release_modes),release_manifest:'sha256',preflight_workflows:Object.keys(contract.release_policy.preflight_workflows||{}),snapshot_latest:snapshot.latest?.date,compare_binding:'data-state/data-amount',mobile_compare:'sticky-first-column',work_pages:Object.keys(workPages),pyeong_pages:Object.keys(sizePages),guide_pages:Object.keys(guidePages),guide_search_routes:12,work_module:'cost-v6'},null,2));
