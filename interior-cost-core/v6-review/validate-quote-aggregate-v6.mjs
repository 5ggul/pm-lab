import {reviewAndAggregate,STANDARD_WORK_ITEMS} from '../quote-ingest/review-aggregate.mjs';
...
const row=(i,{region='서울',grade='A',review='approved',total=5000+(i%12)*40,id=`s${String(i).padStart(4,'0')}`,received=`2026-09-${String((i%28)+1).padStart(2,'0')}T00:00:00Z`}={})=>({
  submission_id:id,schema_version:'1.0',quote_month:'2026-09',region_level1:region,pyeong_band:'30-34평',building_age_band:'20-29년',scope:'전체',bathroom_count:'2',window_status:'excluded',vat_status:'included',waste_status:'included',total_amount_manwon:total,work_items_json:JSON.stringify(work(total)),source_type:'self_reported_quote',quality_grade:grade,quality_flags_json:'[]',review_status:review,received_at:received
});
...
records.push(row(901,{id:'quality-c-0001',grade:'C'}));
records.push(row(902,{id:'pending-0001',review:'pending'}));
for(let i=0;i<79;i++)records.push(row(1000+i,{id:`g${i}`,region:'경기',total:4600+(i%10)*30}));
...
if(publicData.segments.some(x=>x.dimensions.region_level1==='경기'))errors.push('published-below-threshold');
...
for(const id of ['dup-0001','outlier-0001','pending-0001','s0000'])if(publicText.includes(id))errors.push(`public-leaks-id:${id}`);
if(publicText.includes('received_at')||publicText.includes('fingerprint')||publicText.includes('duplicate_of'))errors.push('public-leaks-review-metadata');
...
console.log(JSON.stringify({ok:true,synthetic_only:true,input_count:records.length,published_segments:publicData.segments.length,published_sample_count:seg.sample_count,below_threshold_segment_hidden:true,duplicate_excluded:true,outlier_excluded:true,pending_not_published:true,quality_c_not_published:true,raw_ids_public:false,work_item_statistics:Object.keys(seg.work_items).length},null,2));
