...
export function classifyCollectionError(value){
  ...
  if(msg.includes('SERVICE_KEY_IS_NOT_REGISTERED_ERROR')||msg.includes('등록되지 않은 서비스키'))return{code:'service_key_not_registered',scope:'credential_or_service_authorization',owner_action:'공공데이터포털에서 현재 GitHub Secret의 인증키가 유효한지 확인하고 데이터 ID 15129415 활용신청이 완료된 키로 교체'};
  if(msg.includes('SERVICE_ACCESS_DENIED_ERROR')||msg.includes('PERMISSION_DENIED'))return{code:'service_access_denied',scope:'service_authorization',owner_action:'공공데이터포털에서 데이터 ID 15129415 활용신청·승인·일시중지 상태 확인'};
  if(msg.includes('DEADLINE_HAS_EXPIRED_ERROR'))return{code:'service_key_expired',scope:'credential',owner_action:'공공데이터포털에서 인증키 이용기간 갱신 후 GitHub Secret 교체'};
  ...
  if(msg.includes('HTTP 403'))return{code:'http_403_unclassified',scope:'gateway_or_authorization',owner_action:'sanitized response와 gateway probe를 확인해 권한·키·IP 차단을 구분'};
  return{code:'collection_error_unclassified',scope:'collector_or_source',owner_action:'sanitized collection error를 확인하고 요청 파라미터·원천 상태를 점검'};
}
...
export async function probeGateway({fetchImpl=fetch,apiBase=API_BASE,scheme='https:',start='20260101',end='20260131'}={}){
  ...
  try{
    const res=await fetchImpl(u,{redirect:'manual',headers:{accept:'application/json','user-agent':'interior-cost-gateway-probe/6.3'}});
  ...
  }catch(error){return {scheme:u.protocol,status:0,ok:false,error:String(error?.message||error).slice(0,300),service_key_sent:false}}
}
...
export function buildPreflightReport(dataset,{minRows=5000,maxAgeDays=730,now=new Date(),range=null,collection=null,gatewayProbes=[]}={}){
  ...
  return {
    ...
    blocker:check.ok?null:{code:'publication_readiness_blocked',scope:'dataset_quality',owner_action:'validator errors를 확인하고 조회범위·완전성·신선도 조건을 충족한 뒤 다시 검수'},
    ...
    next_action:check.ok?'snapshot is eligible for the publication pipeline after owner approval':'inspect validator errors; widen the query window or investigate source coverage before publishing'
  };
}
...
export async function main(env=process.env){
  ...
  const perPage=Math.max(1,Math.min(9999,Number(env.PPS_PER_PAGE)||1000));
  ...
  try{
    const collected=await collectLiveRows({apiKey:env.DATA_GO_KR_SERVICE_KEY,start:range.start,end:range.end,perPage,maxPages,minPublishedRows:1});
  ...
  console.log(JSON.stringify({ok:report.validator.ok,mode:report.mode,readiness:report.summary.readiness,blocker:report.blocker,published_rows:report.summary.published_rows,minimum_rows:minRows,latest_published_date:report.summary.latest_published_date,gateway_probes:report.gateway_probes,report:reportFile,snapshot:dataset?snapshotFile:null,repository_write:false,production_deploy:false},null,2));
