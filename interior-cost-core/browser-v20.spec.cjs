...
async function inspect(browser,vp){
  ...
  fs.writeFileSync(path.join(outDir,`${vp.name}-audit.json`),JSON.stringify({version:'20.0.0',checked:rows.length,issues,rows},null,2));return {rows,issues};
}
