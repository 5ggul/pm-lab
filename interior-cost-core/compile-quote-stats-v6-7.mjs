...
export const TRADE_FIELDS=[...NUMERIC].filter(x=>x.endsWith('_manwon')&&x!=='total_amount_manwon');
...
function segmentCells(rows,keyFn,threshold=SEGMENT_N){
  ...
  return [...map.entries()].sort((a,b)=>String(a[0]).localeCompare(String(b[0]),'ko')).map(([key,group])=>({key,n:group.length,status:group.length>=threshold?'published':'withheld',needed:Math.max(0,threshold-group.length),...(group.length>=threshold?{distribution:distributionFor(group)}:{reason:`N<${threshold}`})}));
}
export function aggregateRows(rows,{reviewedOn=new Date().toISOString().slice(0,10)}={}){
  ...
  return {
    ...
    metrics_visible:eligible?['median','p25','p75']:[],reason:eligible?'검수 통과 표본 수 기준 충족':`검수 통과 실제 견적 표본 N=${n}, 공개 기준 N>=${PUBLIC_N} 미달`,needed_for_overall:Math.max(0,PUBLIC_N-n),
    ...
    overall:eligible?{n,distribution:distributionFor(rows)}:{n,status:'withheld',needed:Math.max(0,PUBLIC_N-n)},
    segments:{
...
function health(rows,errors,header,{reviewedOn}){
  ...
  for(const f of TRADE_FIELDS){const n=rows.filter(r=>r[f]!=null&&Number.isFinite(Number(r[f]))).length;counts[f]=n;percent[f]=rows.length?round(n/rows.length*100):0}
  return {version:VERSION,reviewed_on:reviewedOn,source_mode:rows.length?'private_csv_aggregate_only':'no_private_dataset',valid_rows:rows.length,invalid_messages:errors.length,header_columns:header.length,raw_rows_persisted:false,raw_rows_written_to_docs:false,trade_field_counts:counts,trade_field_completion_percent:percent};
}
export function compileQuoteStats({inputPath=null,csvText=null,statsPath=DEFAULT_STATS,healthPath=DEFAULT_HEALTH,reviewedOn=new Date().toISOString().slice(0,10),strict=true}={}){
  ...
  const stats=blocked?{version:VERSION,reviewed_on:reviewedOn,sample_count:0,status:'blocked_invalid_rows',publication_threshold:PUBLIC_N,segment_threshold:SEGMENT_N,metrics_visible:[],reason:'입력 검증 오류가 있어 통계를 생성하지 않음',needed_for_overall:PUBLIC_N,overall:{n:0,status:'withheld',needed:PUBLIC_N},segments:{}}:aggregateRows(rows,{reviewedOn});
...
function selfTest(){
  ...
  for(let i=0;i<40;i++){
    const row={sample_id:`T${String(i).padStart(3,'0')}`,quote_month:'2026-08',region_level1:i<25?'서울':'경기',supply_pyeong:32,exclusive_pyeong:25.7,building_type:'아파트',scope:'올수리',bathroom_count:2,window_scope:'제외',vat_state:'포함',waste_state:'포함',total_amount_manwon:4000+i*10,bathroom_manwon:i<30?700+i:null};
  ...
  const out=aggregateRows(parsed.rows,{reviewedOn:'2026-09-11'});if(out.status!=='published'||out.overall.distribution.total_amount_manwon.median!==4195||out.needed_for_overall!==0)throw new Error('self-test overall failed');
  const seoul=out.segments.region_level1.find(x=>x.key==='서울'),gyeonggi=out.segments.region_level1.find(x=>x.key==='경기');if(seoul?.status!=='published'||gyeonggi?.status!=='withheld'||gyeonggi?.needed!==5)throw new Error('self-test threshold failed');
  ...
  const tmpStats=path.join('/tmp','interior-v11-self-stats.json'),tmpHealth=path.join('/tmp','interior-v11-self-health.json');const compiled=compileQuoteStats({csvText:lines.join('\n'),statsPath:tmpStats,healthPath:tmpHealth,reviewedOn:'2026-09-11'});if(compiled.health.trade_field_counts.bathroom_manwon!==30||compiled.health.trade_field_completion_percent.bathroom_manwon!==75)throw new Error('self-test trade coverage failed');
