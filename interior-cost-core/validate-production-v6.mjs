...
for(const file of htmlFiles){
 const rel=path.relative(root,file).replaceAll(path.sep,'/'),html=fs.readFileSync(file,'utf8'),is404=rel==='404.html';
 ...
 if(is404){if(!/name=["']robots["'][^>]+noindex/i.test(html))errors.push('404-indexable');continue}
...
let sitemapCount=0;if(fs.existsSync(sitemapPath)){const sm=fs.readFileSync(sitemapPath,'utf8');sitemapCount=(sm.match(/<url>/g)||[]).length;if(sitemapCount!==htmlFiles.length-1)errors.push(`sitemap-count:${sitemapCount}/${htmlFiles.length-1}`);if(sm.includes('/404.html'))errors.push('sitemap-404')}
...
console.log(JSON.stringify({ok:true,html_count:htmlFiles.length,indexable_count:htmlFiles.length-1,unique_titles:titles.size,unique_descriptions:descs.size,sitemap_count:sitemapCount,static_trust_links:trustTargets.length,base_url:base.href},null,2));
