...
const OPERATIONS=[
  ['building_material','getPriceInfoListFcltyCmmnMtrilBildng'],
  ['building_market_construction','getPriceInfoListMrktCnstrctPcBildng'],
...
for(const [id,operation] of OPERATIONS){
  ...
  for(const [k,v] of Object.entries({ServiceKey:serviceKey,pageNo:'1',numOfRows:'1',type:'json'}))u.searchParams.set(k,v);
  try{
    ...
    output.push({
      id,operation,http_status:r.status,result_code:code,result_message:message,
      total_count:Number(body.totalCount||0),sample_fields:Object.keys(sample).sort()
    });
  }catch(error){
