...
test('integrity page renders three sources without automatic price judgment on desktop and mobile',async({page})=>{
  for(const viewport of [{width:1440,height:900},{width:390,height:844}]){await page.setViewportSize(viewport);await page.goto(url('data/g2b-data-integrity/index.html'),{waitUntil:'domcontentloaded'});await expect(page.locator('[data-v20-integrity-page]')).toHaveCount(1);await expect(page.locator('[data-v20-integrity-card]')).toHaveCount(3);await expect(page.locator('meta[name="robots"]')).toHaveAttribute('content',/noindex,nofollow/);const text=await page.locator('[data-v20-integrity-page]').innerText();expect(text).toContain('SHA-256');expect(text).toContain('3 / 3 통과');expect(text).toContain('보증이 아닙니다');expect(text).not.toContain('판정 결과');expect(await page.evaluate(()=>document.documentElement.scrollWidth>window.innerWidth+2)).toBe(false);for(const card of await page.locator('[data-v20-integrity-card]').all()){const code=await card.locator('code').first().innerText();expect(code).toMatch(/^[0-9a-f]{64}$/)}}
