...
fs.writeFileSync(path.join(outDir,'init.json'),JSON.stringify({version:audit.version,started:true},null,2));
...
test('quote paste parses currency after quantity and hands amount-only state to quote check',async({page})=>{
  ...
  const publicRow=page.locator('[data-v20-qb-check-row="bathroom"]');await expect(publicRow).toHaveCount(1);await expect(publicRow.locator('[data-v20-qb-ref]')).toHaveCount(0);await expect(publicRow).toContainText('수량과 단위를 입력하세요');
  await row.locator('[data-q-qty]').fill('10');await row.locator('[data-q-unit]').fill(bathroomRef.unit_key);await expect(publicRow.locator('[data-v20-qb-ref]').first()).toHaveCount(1);
