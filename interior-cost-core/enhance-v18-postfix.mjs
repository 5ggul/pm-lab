...
const journey=json('data/release-journey-v18.json',{isolated:99,missing_next_actions:99,rows:[]});
...
for(const x of release.urls||[]){
  if(!exists(x.path)){releaseBroken.push({from:x.path,href:'missing page',target:x.path});continue}
  const h=read(x.path);
  for(const m of h.matchAll(/href="([^"]+)"/g)){
    const p=internalTarget(m[1]);
    if(p&&(m[1].startsWith('/')||m[1].startsWith(BASE)||m[1].startsWith(SITE))&&!exists(p))releaseBroken.push({from:x.path,href:m[1],target:p});
  }
}
const finalJourney={...journey,broken_internal_links:releaseBroken.length,final_recheck:true};
write('data/release-journey-v18.json',JSON.stringify(finalJourney,null,2));
...
if(failed.length)throw new Error(`v18 postfix gate failed ${JSON.stringify({failed,quality,releaseBroken})}`);
console.log(JSON.stringify({version:VERSION,quality,checks,releaseBroken:releaseBroken.length},null,2));
