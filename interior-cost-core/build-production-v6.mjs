...
function breadcrumb(rel,title){
  const parts=rel==='index.html'?[]:rel.replace(/\/index\.html$/,'').split('/').filter(Boolean);
  if(!parts.length)return null;
  const labels={guides:'가이드',cost:'공사별 비용','interior-cost':'평수별 비용','quote-check':'견적 검사','quote-compare':'견적 비교',calculator:'계산기',checklist:'체크리스트',glossary:'용어사전',standards:'표준 공종','data-method':'데이터 기준',changelog:'변경이력',about:'소개',contact:'연락',privacy:'개인정보',terms:'이용약관',disclaimer:'면책',data:'데이터'};
  const items=[{name:'홈',item:new URL('',base).href}];
  let p='';
  parts.forEach((part,i)=>{p+=`${part}/`;items.push({name:i===parts.length-1?title.replace(/\s*\|.*$/,''):labels[part]||part,item:new URL(p,base).href})});
  return {'@type':'BreadcrumbList',itemListElement:items.map((x,i)=>({'@type':'ListItem',position:i+1,name:x.name,item:x.item}))};
}
function schemas(rel,title,description,url){
  const list=[];
  const bc=breadcrumb(rel,title);
  if(bc)list.push(bc);
  if(rel==='index.html'){
    list.push({'@type':'Organization',name:'견적검수실',url});
    list.push({'@type':'WebSite',name:'견적검수실',url,description});
  }else if(/^(quote-check|quote-compare|calculator|checklist)\/index\.html$/.test(rel)){
    list.push({'@type':'WebApplication',name:title.replace(/\s*\|.*$/,''),url,description,applicationCategory:'UtilitiesApplication',operatingSystem:'Web browser',offers:{'@type':'Offer',price:'0',priceCurrency:'KRW'}});
  }else if(rel.startsWith('guides/')){
    list.push({'@type':'Article',headline:title.replace(/\s*\|.*$/,''),description,url,dateModified:'2026-09-10',inLanguage:'ko-KR',author:{'@type':'Organization',name:'견적검수실'}});
  }else if(rel==='data/construction-cost-index/index.html'){
    list.push({'@type':'Dataset',name:title.replace(/\s*\|.*$/,''),description,url,inLanguage:'ko-KR',creator:{'@type':'Organization',name:'견적검수실'},dateModified:'2026-09-10'});
  }
...
for(const file of htmlFiles){
  ...
  html=html.replace(/<meta[^>]+name=["']robots["'][^>]*>/i,is404?'<meta name="robots" content="noindex,nofollow">':'<meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1">');
...
console.log(JSON.stringify({ok:true,base_url:base.href,html_count:htmlFiles.length,indexable_count:urls.length,output:out},null,2));
