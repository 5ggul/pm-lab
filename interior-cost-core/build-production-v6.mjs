...
function breadcrumb(rel,title){const parts=rel==='index.html'?[]:rel.replace(/\/index\.html$/,'').split('/').filter(Boolean);if(!parts.length)return null;const labels={guides:'가이드',cost:'공사별 비용','interior-cost':'평수별 비용','quote-check':'견적 검사','quote-compare':'견적 비교',calculator:'계산기',checklist:'체크리스트',glossary:'용어사전',standards:'표준 공종','data-method':'데이터 기준',changelog:'변경이력',about:'소개',contact:'연락',privacy:'개인정보',terms:'이용약관',disclaimer:'면책',data:'데이터'};const items=[{name:'홈',item:new URL('',base).href}];let p='';parts.forEach((part,i)=>{p+=`${part}/`;items.push({name:i===parts.length-1?title.replace(/\s*\|.*$/,''):labels[part]||part,item:new URL(p,base).href})});return {'@type':'BreadcrumbList',itemListElement:items.map((x,i)=>({'@type':'ListItem',position:i+1,name:x.name,item:x.item}))}}
...
for(const file of htmlFiles){
  ...
  const url=pageUrl(rel);const is404=rel==='404.html';
