...
function breadcrumb(rel,title){const parts=rel==='index.html'?[]:rel.replace(/\/index\.html$/,'').split('/').filter(Boolean);if(!parts.length)return null;const labels={guides:'가이드',cost:'공사별 비용','interior-cost':'평수별 비용','quote-check':'견적 검사','quote-compare':'견적 비교','quote-submit':'익명 견적','work-match':'공종 매칭',statistics:'통계','reference-prices':'공공 단가',calculator:'계산기',checklist:'체크리스트',glossary:'용어사전',standards:'표준 공종','data-method':'데이터 기준',changelog:'변경이력',about:'소개',contact:'연락',privacy:'개인정보',terms:'이용약관',disclaimer:'면책',data:'데이터'};const items=[{name:'홈',item:new URL('',base).href}];let p='';parts.forEach((part,i)=>{p+=`${part}/`;items.push({name:i===parts.length-1?title.replace(/\s*\|.*$/,''):labels[part]||part,item:new URL(p,base).href})});return {'@type':'BreadcrumbList',itemListElement:items.map((x,i)=>({'@type':'ListItem',position:i+1,name:x.name,item:x.item}))}}
function schemas(rel,title,description,url){const list=[];const bc=breadcrumb(rel,title);if(bc)list.push(bc);if(rel==='index.html'){list.push({'@type':'Organization',name:'견적검수실',url});list.push({'@type':'WebSite',name:'견적검수실',url,description})}else if(/^(quote-check|quote-compare|calculator|checklist|work-match)\/index\.html$/.test(rel)){list.push({'@type':'WebApplication',name:title.replace(/\s*\|.*$/,''),url,description,applicationCategory:'UtilitiesApplication',operatingSystem:'Web browser',offers:{'@type':'Offer',price:'0',priceCurrency:'KRW'}})}else if(rel==='statistics/index.html'){list.push({'@type':'WebApplication',name:'인테리어 견적 통계',url,description,applicationCategory:'UtilitiesApplication',operatingSystem:'Web browser',offers:{'@type':'Offer',price:'0',priceCurrency:'KRW'}});list.push({'@type':'Dataset',name:'익명 인테리어 견적 공개 집계',description:'동일 조건의 검수 승인 익명 견적이 세그먼트별 최소 80건을 충족할 때 P25·중앙값·P75와 공종별 분포를 공개하는 집계 데이터입니다.',url:new URL('data/quote-public-segments.json',base).href,inLanguage:'ko-KR',creator:{'@type':'Organization',name:'견적검수실'},measurementTechnique:'세그먼트별 검수 승인 표본 집계, 정확 중복 제거, MAD 기반 이상치 분리, 최소 공개 표본 80건'})}else if(rel==='reference-prices/index.html'){list.push({'@type':'WebApplication',name:'공공 단가 조회',url,description,applicationCategory:'UtilitiesApplication',operatingSystem:'Web browser',offers:{'@type':'Offer',price:'0',priceCurrency:'KRW'}});list.push({'@type':'Dataset',name:'공공 공사비 참고단가',description:'조달청 공사원가 표준시장단가의 품명·규격·단위·재료비·노무비·경비·합계·적용조건을 검색용으로 정규화한 참고 데이터입니다.',url:new URL('data/public-unit-prices.json',base).href,inLanguage:'ko-KR',creator:{'@type':'Organization',name:'조달청'},isBasedOn:'https://www.data.go.kr/data/15151298/fileData.do',measurementTechnique:'조달청 공개 표준시장단가 원본을 필드 정규화하여 제공. 민간 인테리어 시장가격으로 변환하지 않음'})}else if(rel.startsWith('guides/')){list.push({'@type':'Article',headline:title.replace(/\s*\|.*$/,''),description,url,dateModified:'2026-09-11',inLanguage:'ko-KR',author:{'@type':'Organization',name:'견적검수실'}})}else if(rel==='data/construction-cost-index/index.html'){list.push({'@type':'Dataset',name:title.replace(/\s*\|.*$/,''),description,url,inLanguage:'ko-KR',creator:{'@type':'Organization',name:'견적검수실'},dateModified:'2026-09-11'})}return {'@context':'https://schema.org','@graph':list}}
...
const xml=`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.map(u=>`  <url><loc>${esc(u)}</loc><lastmod>2026-09-11</lastmod></url>`).join('\n')}\n</urlset>\n`;
