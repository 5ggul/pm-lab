...
for(const f of html){const h=fs.readFileSync(f,'utf8');const p=path.relative(ROOT,f).split(path.sep).join('/');if(!h.includes('noindex'))problems.push(`${p}: noindex missing`);if(!h.includes('site-v33-stratton-visual.css'))problems.push(`${p}: v33 css missing`);if(!h.includes('v33-stratton'))problems.push(`${p}: body class missing`);if(!h.includes('v33-ticker'))problems.push(`${p}: ticker missing`);if(visitorMedia(p)&&!h.includes('v33-page-media'))problems.push(`${p}: visitor page media missing`);if(/site-v32-stratton|v32-stratton|site-v30-hyper|v30-hyper/.test(h))problems.push(`${p}: discarded design residue`)}
...
const result={version:'33.0.1',html:html.length,visitor_media_pages:html.map(f=>path.relative(ROOT,f).split(path.sep).join('/')).filter(visitorMedia).length,problems,pass:problems.length===0};fs.writeFileSync(path.join(ROOT,'data/v33-stratton-visual-validation.json'),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));if(problems.length)process.exit(1);