...
async function fetchTextOnce(url,{fetchImpl=fetch,timeoutMs=20000,headers={}}={}){
 ...
 try{const response=await fetchImpl(url,{signal:controller.signal,headers:{'user-agent':'pm-lab-franchise-data-core/1.3',accept:'application/json, application/xml;q=0.8, text/xml;q=0.7',...headers}});return {ok:response.ok,status:response.status,text:await response.text(),headers:response.headers,url:String(url)}}finally{clearTimeout(timer)}
}
export async function fetchText(url,{attempts=2,...opts}={}){
 let last;
 for(let i=0;i<Math.max(1,attempts);i++){
  try{return await fetchTextOnce(url,opts)}catch(e){last=e;if(i+1<attempts)await sleep(700*(i+1))}
 }
 throw last;
}
