-- Avoid readiness flapping at the rolling 24-hour boundary.
-- Require at least 23 hourly buckets in the last 24h window while retaining
-- the trusted coverage and average coverage quality gates.
...
select
  ...
  (char_length(trim(game.description_ko)) >= 80) as has_editorial_description,
  (
  ...
  (state.freshness_state = 'fresh' and state.playing is not null) as current_data_available
from public.games as game
left join public.game_provider_state as state on state.universe_id = game.universe_id
left join public.collector_targets as target on target.universe_id = game.universe_id
left join public.game_enrichment as enrichment on enrichment.universe_id = game.universe_id
left join public.game_rollups_hourly as hourly on hourly.universe_id = game.universe_id
group by
  game.universe_id, game.canonical_slug, game.index_state, game.description_ko,
  state.fetched_at, state.freshness_state, state.playing,
  target.cadence_minutes, enrichment.hero_image_url;
