...
export async function checkReviewPublic({browser,base}) {
 ...
 try {
  ...
  for(const [game,guide] of [["blox-fruits","fruit-basics"],["fisch","fishing-controls"],["grow-a-garden","planting-basics"],["rivals","first-duel"],["murder-mystery-2","roles"]]){
   ...
   const reading=await page.locator('.guide-reading').boundingBox(),cta=await page.locator('.guide-question-cta').boundingBox(),media=await page.locator('.guide-media-section').boundingBox();assert.ok(reading&&cta&&cta.y>=reading.y+reading.height-2&&(!media||cta.y<media.y));assert.equal(await page.locator('.guide-question-cta a').getAttribute('href'),`/game/${game}/questions`);
