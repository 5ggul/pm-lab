...
test("low-baseline percentage explosion does not dominate a meaningful larger move",()=>{
  const small=computeTrend(1,series(1,20),"2026-09-18T00:00:00Z",new Date("2026-09-18T01:00:00Z"));
  const large=computeTrend(2,series(1000,1500),"2026-09-18T00:00:00Z",new Date("2026-09-18T01:00:00Z"));
...
test("missing rows are counted as missing expected time slots",()=>{
  ...
  const r=computeTrend(4,sparse,"2026-09-18T00:00:00Z",new Date("2026-09-18T01:00:00Z"),60);
  assert.equal(r.eligible,false);
  assert.equal(r.confidence,"insufficient");
});
...
test("six-hour preview fixture cadence remains eligible when declared explicitly",()=>{
  ...
  const r=computeTrend(5,points,"2026-09-18T00:00:00Z",new Date("2026-09-18T01:00:00Z"),360);
  assert.equal(r.eligible,true);
  assert.ok((r.metrics.coverageRatio??0)>.99);
  assert.equal(r.calculationVersion,"trend_v1_2");
});
