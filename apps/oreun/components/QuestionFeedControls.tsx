...
export default function QuestionFeedControls({ path, filters, games, showScope = true }: {
  path: string;
  filters: FeedFilters;
  games?: Array<{ slug: string; nameKo: string }>;
  showScope?: boolean;
}) {
  return <div className={styles.toolbar}>
    <nav aria-label="질문 보기" className={styles.tabs}>
      {([['latest', '최근 질문'], ['unanswered', '답변 없는 질문'], ['resolved', '해결된 질문']] as const).map(([state, label]) => (
        <Link key={state} href={feedHref(path, { ...filters, state, page: 1 })} aria-current={filters.state === state ? "page" : undefined}>{label}</Link>
      ))}
    </nav>
    {(games || showScope) && <form action={path} method="get" className={styles.filters}>
      <input type="hidden" name="state" value={filters.state} />
      {games ? <label>게임
        <select name="game" aria-label="게임" defaultValue={filters.game}>
          <option value="">모든 게임</option>
          {games.map(game => <option key={game.slug} value={game.slug}>{game.nameKo}</option>)}
        </select>
      </label> : filters.game && <input type="hidden" name="game" value={filters.game} />}
      {showScope && <label>관심 범위
        <select name="scope" aria-label="관심 범위" defaultValue={filters.scope}>
          <option value="all">전체 게임</option><option value="following">내 관심 게임</option>
        </select>
      </label>}
