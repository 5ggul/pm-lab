...
export default function CommunityAccess({access,next,mode="question"}:{access:WriteAccess;next:string;mode?:"question"|"answer"|"party"|"post"|"comment"}){
 if(access==="ready")return null;
 const noun=mode==="party"?"모집 글":mode==="answer"?"답변":mode==="post"?"자유글":mode==="comment"?"댓글":"질문";
 const details={
