...
export default function DraftForm({userId,scope,kind,initialRequestId,fields,action,children,submitLabel}:{userId:string;scope:string;kind:"comment"|"party"|"freePost"|"freeComment"|"communityGuide";initialRequestId:string;fields:string[];action:(data:FormData)=>Promise<WriteResult>;children:ReactNode;submitLabel:string}) {
  ...
  function persist(nextRequestId?:string){if(!formRef.current||committed.current)return;try{const data=new FormData(formRef.current);const values=Object.fromEntries(fields.map(name=>{const el=formRef.current?.elements.namedItem(name);return [name,el instanceof HTMLInputElement||el instanceof HTMLTextAreaElement||el instanceof HTMLSelectElement?el.value.slice(0,5000):""];}));sessionStorage.setItem(key,JSON.stringify({userId,scope,kind,requestId:nextRequestId??String(data.get("request_id")),values,savedAt:Date.now()}));}catch{}}
  ...
  function startNewRequest(){if(pending||lock.current)return;const id=crypto.randomUUID();committed.current=false;setRequestId(id);setResult(null);persist(id);}
  ...
  return <form ref={formRef} onSubmit={submit} onInput={()=>{committed.current=false;persist();}} onChange={()=>{committed.current=false;persist();}} className="stack-form resilient-form" aria-busy={pending} data-testid={kind+"-composer"}>
    ...
    {result&&result.status!=="conflict"&&<div role={result.status==="success"?"status":"alert"} className="callout">{result.message}{result.href&&result.status!=="success"&&<p><a href={result.href}>확인하고 돌아오기 →</a></p>}</div>}
    <WriteConflict result={result} disabled={pending} onNewRequest={startNewRequest}/>
