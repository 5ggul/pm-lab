...
export default function Header({ games = [] }: { games?: GameView[] }) {
  return <header className="site-header">
    <div className="nav-shell">
      ...
      <details className="mobile-menu"><summary>메뉴 <span aria-hidden="true">☰</span></summary><nav aria-label="전체 메뉴"><Link prefetch={false} href="/community/free">자유</Link><Link prefetch={false} href="/community">질문답변</Link><Link prefetch={false} href="/guides">공략</Link><Link prefetch={false} href="/updates">업데이트</Link><Link prefetch={false} href="/games?intent=party">파티 모집</Link><Link prefetch={false} href="/games">게임 찾기</Link><Link prefetch={false} href="/rising">상승 중</Link><Link prefetch={false} href="/compare">비교</Link><Link prefetch={false} href="/notifications">알림</Link></nav></details>
    </div>
