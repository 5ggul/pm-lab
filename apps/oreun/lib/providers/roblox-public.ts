...
export class RobloxPublicGamesProvider implements GameProvider {
  ...
  async getGames(universeIds:number[]):Promise<ProviderGame[]>{
    ...
    try{const res=await fetch(url,{headers:{Accept:"application/json","User-Agent":"Oreun-R1-Preview/0.1"},signal:controller.signal,next:{revalidate:120}});
      ...
      return (json.data??[]).map(g=>({universeId:g.id,rootPlaceId:g.rootPlaceId,name:g.name,description:g.description??"",creatorName:g.creator?.name??"알 수 없음",playing:Number.isFinite(g.playing)?g.playing!:null,visits:Number.isFinite(g.visits)?g.visits!:null,favorites:Number.isFinite(g.favoritedCount)?g.favoritedCount!:null,sourceUpdatedAt:g.updated??null,fetchedAt,sourceProvider:"roblox_public_games",sourceEndpoint:this.endpoint,sourceClass:"ROBLOX_PUBLIC_API",sourceStatus:"live"}));
