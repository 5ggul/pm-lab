...
export function answerDraftKey(userId: string, questionId: string) {
  return PREFIX + userId + ":" + questionId;
}
...
export function loadAnswerDraft(
  ...
  const key = answerDraftKey(userId, questionId);
  ...
  try {
    ...
    const valid =
      ...
      now - data.savedAt < ANSWER_DRAFT_TTL_MS;
    if (!valid) {
...
export function persistAnswerDraft(
  storage: Pick<Storage, "setItem" | "removeItem">,
  draft: AnswerDraft,
) {
  const key = answerDraftKey(draft.userId, draft.questionId);
  if (!draft.body) storage.removeItem(key);
