...
export default async function PrivacyPage(){const games=await getGameCatalog();return <InfoPage games={games} title="개인정보 처리 안내" intro="시행 기준일 2026-09-23 · 현재 서비스 구조 기준입니다.">
 ...
 <h2>처리 목적과 보유·삭제 기준</h2><p>로그인 식별 정보는 계정 인증에, 공개 프로필과 작성글은 커뮤니티 제공에, 연령 자기 확인과 계정 상태는 작성 권한·안전 관리에 사용합니다. 신고와 필요한 운영 기록은 게시물 검토에 사용합니다.</p><p>계정과 커뮤니티 자료는 해당 기능을 제공하는 동안 저장합니다. 탈퇴 시의 서비스 DB 처리와 같은 탭의 24시간 초안 처리 방식은 위 안내를 따릅니다. 호스팅 백업·보안 로그의 실제 보관 기간과 별도 보존이 필요한 항목은 운영자가 확인하여 공개 전에 확정해야 합니다. 확인되지 않은 기간을 정해진 기간처럼 안내하지 않습니다.</p>
 <h2>이용자의 확인·정정·삭제 요청</h2><p><a href="/me">내 정보</a>에서 공개 프로필을 확인·수정하고 회원 탈퇴를 진행할 수 있습니다. 그 밖의 개인정보 열람·정정·삭제·처리정지 관련 문의는 <a href="/contact#private">비공개 문의 안내</a>를 확인해 주세요. 창구가 미설정 상태라면 운영자의 설정이 필요합니다. 이메일이나 계정 정보를 GitHub 공개 이슈에 올리지 마세요.</p>
 <h2>외부 서비스와 처리 관계</h2><p>Google은 로그인 인증에, Supabase는 인증·데이터베이스·서버 함수 처리에 사용합니다. 현재 프리뷰는 Vercel에서 제공하며 Cloudflare는 데이터 수집·보조 호스팅에 사용됩니다. 게임 이미지·영상 등을 열면 해당 제공자에 요청이 전달될 수 있습니다. 실제 계약상 처리 위탁 관계, 보관 지역·국외 이전, 보안 로그 보관 설정은 운영자가 확인하여 공개 전에 구체화해야 합니다.</p>
 </InfoPage>;}
