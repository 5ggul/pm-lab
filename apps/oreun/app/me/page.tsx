...
export default async function MePage({searchParams}:{searchParams:Promise<{error?:string;saved?:string;welcome?:string;next?:string}>}){
 ...
 {params.welcome==="google"&&!permissions.age_confirmed_14_plus?<div className="callout"><strong>Google 로그인이 완료됐습니다.</strong><br/>커뮤니티 기능을 사용하려면 공개 프로필을 확인하고 만 14세 이상임을 확인해 주세요. 확인 전에는 질문·답변·댓글·파티 작성 권한이 열리지 않습니다.</div>:params.welcome?<div className="callout">가입이 완료됐습니다. 공개 프로필 이름을 설정해 주세요.</div>:null}
 <div className="status-grid"><div className="status-cell"><strong>{permissions.active?"정상":"제한"}</strong><span>계정 상태</span></div><div className="status-cell"><strong>{permissions.age_confirmed_14_plus?"완료":"필요"}</strong><span>만 14세 이상 확인</span></div><div className="status-cell"><strong>{permissions.role}</strong><span>권한</span></div><div className="status-cell"><strong>{followedGames.length}</strong><span>팔로우 게임</span></div></div>
 <section className="panel profile-panel"><h2>공개 프로필</h2><form action={updateProfileAction} className="stack-form"><input type="hidden" name="next" value={onboardingNext}/><label>아이디<input name="handle" defaultValue={profile.handle} minLength={3} maxLength={20} pattern="[a-z0-9_]+" required/><small>영문 소문자·숫자·밑줄 3~20자</small></label><label>표시 이름<input name="display_name" defaultValue={profile.display_name??""} maxLength={30}/></label><label>소개<textarea name="bio" defaultValue={profile.bio} maxLength={300} rows={4}/></label><label className="check-line"><input type="checkbox" name="age_confirmed_14_plus" defaultChecked={permissions.age_confirmed_14_plus}/><span>만 14세 이상임을 확인합니다.</span></label><button className="primary-button" type="submit">프로필 저장</button></form></section>
