...
const toolsBasis=`<section class="v25-method" data-v26-seo-floor="tools"><h2>기준</h2><p>계산 도구는 사용자가 입력한 금액과 비율을 그대로 계산하며, 브랜드·업종 데이터는 공정거래위원회 공개자료를 Tier A/B 신뢰 기준으로 정규화한 동일 스냅샷을 사용합니다. 공개 창업비용·평균매출·가맹점 수·3.3㎡당 매출은 사용자 입력값과 섞어 공식 평균처럼 표시하지 않습니다.</p><p>업종 중앙값과 분포는 같은 품질 기준을 통과한 공개값만 비교하고, 누락값은 0으로 채우지 않습니다. 0으로 공개된 값은 원자료의 0으로 보존하되 무료·면제로 해석하지 않습니다. 평균매출과 3.3㎡당 매출은 이익 또는 예상 수익을 뜻하지 않습니다.</p><p>각 결과의 기준연도·표본·포함 항목·제외 항목은 해당 도구 하단과 <a href="/pm-lab/franchise-ssg-preview/methodology/">산정방법</a>, <a href="/pm-lab/franchise-ssg-preview/sources/">출처</a>에서 확인할 수 있습니다. 지역매출은 지역 단위 원자료가 단일 스냅샷에 검증·병합되기 전까지 노출하지 않습니다.</p></section>`;
...
const report={schemaVersion:1,uiVersion:'11.26',generatedAt:new Date().toISOString(),routes:['/tools/','/tools/monthly-profit-simulator/'],policy:'TOP_COPY_STAYS_FUNCTIONAL; EXPLANATION_ONLY_IN_LOWER_BASIS; NO_NEW_ROUTE; NO_INDEX_CHANGE'};
