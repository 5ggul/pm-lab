...
const errors=[];
const report=JSON.parse(await fs.readFile(path.join(out,'v11-20-internal-authority.json'),'utf8'));
...
if(manifest.v11_20?.internalAuthorityPolicy!==true)errors.push('manifest internalAuthorityPolicy flag missing');
if(manifest.v11_20?.budgetExplorerPreserved!==true)errors.push('manifest budgetExplorerPreserved flag missing');
if(quality.contentTrust?.version!=='11.20'||quality.contentTrust?.internalAuthorityPolicy!==true)errors.push('quality internal authority metadata missing');
if(quality.contentTrust?.budgetExplorerPreserved!==true)errors.push('quality budgetExplorerPreserved metadata missing');
if(report.productionCandidateCount!==182)errors.push(`production candidate count changed: ${report.productionCandidateCount}`);
if(report.productionCandidateCount!==budgetIntent.productionCandidateCount)errors.push(`v11.19/v11.20 candidate mismatch ${budgetIntent.productionCandidateCount}/${report.productionCandidateCount}`);
...
if(report.baselineBudgetExplorer?.productionCandidate!==true)errors.push('v11.19 explore candidate was not preserved');
if(report.baselineBudgetExplorer?.trustedBrands!==budgetIntent.trustedBrands)errors.push('v11.19 trusted brand count changed');

if(!home.includes('data-v11-20-home-paths="1"'))errors.push('home discovery-path section missing');
if(!home.includes('data-v11-20-home-tools="1"'))errors.push('home verified-tool section missing');
...
for(const route of ['/explore/','/brands/','/categories/','/compare/','/tools/']){
  if(!home.includes(`href="/pm-lab/franchise-ssg-preview${route}"`))errors.push(`home discovery link missing: ${route}`);
}
...
if(!explore.includes('Tier A/B'))errors.push('budget explorer trust copy missing');
if(!explore.includes('noindex,nofollow,noarchive,nosnippet'))errors.push('budget explorer preview noindex lost');
if(!categories.includes('data-v11-20-category-directory="1"'))errors.push('category row-directory marker missing');
...
if(!css.includes('/* v11.20 internal authority */'))errors.push('v11.20 authority CSS missing');
...
if(report.home?.exploreLinked!==true)errors.push('home does not directly link /explore/');
if((report.home?.discoveryLinks||[]).length!==5)errors.push(`home discovery link count ${report.home?.discoveryLinks?.length||0}`);
if(report.home?.verifiedToolLinks!==8)errors.push(`home direct verified-tool links ${report.home?.verifiedToolLinks}`);
if(manifest.v11_20?.homeDiscoveryLinks!==5)errors.push(`manifest homeDiscoveryLinks ${manifest.v11_20?.homeDiscoveryLinks}`);
if(manifest.v11_20?.homeVerifiedToolLinks!==8)errors.push(`manifest homeVerifiedToolLinks ${manifest.v11_20?.homeVerifiedToolLinks}`);
if(manifest.v11_20?.hubCoverageComplete!==true)errors.push('manifest hubCoverageComplete is false');
if(manifest.v11_20?.orphanCandidateCount!==0)errors.push(`manifest orphanCandidateCount ${manifest.v11_20?.orphanCandidateCount}`);
...
if(errors.length){
  console.error(JSON.stringify({v11_20Validation:'FAIL',errors,summary:{productionCandidates:report.productionCandidateCount,maxDepth:graph.maxCandidateDepth,exploreDepth:graph.exploreDepth,orphans:graph.orphanCandidates?.length||0,homeVerifiedTools:report.home?.verifiedToolLinks||0}},null,2));
  process.exit(1);
}
console.log(JSON.stringify({v11_20Validation:'PASS',productionCandidates:report.productionCandidateCount,candidateTypeCounts:report.candidateTypeCounts,maxCandidateDepth:graph.maxCandidateDepth,depthDistribution:graph.depthDistribution,exploreDepth:graph.exploreDepth,homeDiscoveryLinks:5,homeVerifiedTools:8,hubCoverage:Object.fromEntries(Object.entries(report.hubCoverage).map(([k,v])=>[k,`${v.linkedCandidates}/${v.expected}`]))},null,2));
