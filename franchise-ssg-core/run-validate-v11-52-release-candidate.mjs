...
import {validateContextualGuides} from './contextual-guides-integrator.mjs';
import {validateBrandEvidence} from './brand-evidence-integrator.mjs';
...
if(report.retentionWorkspaceUx!==true||Number(report.retentionBrandWorkspaces)!==136||Number(report.updatesRadarRows)!==12||report.localOnlyPersistence!==true||report.candidateNotes!==true||report.checklistProgressSummary!==true||report.candidatePlanning!==true||report.shortlistDashboard!==true||report.shortlistBackup!==true||report.changeInbox!==true||Number(report.changeInboxSurfaces)!==2||report.changeAcknowledgement!==true||report.changedOnlyFilter!==true||report.shortlistDecisionBoard!==true||report.shortlistCsvExport!==true||Number(report.editorialRails)!==3||Number(report.editorialGuideLinks)!==9)err.push(`report retention ${report.retentionWorkspaceUx}/${report.retentionBrandWorkspaces}/${report.updatesRadarRows}/${report.localOnlyPersistence}/${report.candidateNotes}/${report.checklistProgressSummary}/${report.candidatePlanning}/${report.shortlistDashboard}/${report.shortlistBackup}/${report.changeInbox}/${report.changeInboxSurfaces}/${report.changeAcknowledgement}/${report.changedOnlyFilter}/${report.shortlistDecisionBoard}/${report.shortlistCsvExport}/${report.editorialRails}/${report.editorialGuideLinks}`);
...
if(report.contextualGuidesUx!==true||Number(report.contextualBrandRails)!==136||Number(report.contextualCategoryRails)!==16||Number(report.contextualHubRails)!==6||Number(report.contextualTotalRails)!==158||Number(report.contextualGuideLinks)!==474||report.contextualCandidateOnly!==true)err.push(`report contextualGuides ${report.contextualGuidesUx}/${report.contextualBrandRails}/${report.contextualCategoryRails}/${report.contextualHubRails}/${report.contextualTotalRails}/${report.contextualGuideLinks}/${report.contextualCandidateOnly}`);
if(report.brandEvidenceUx!==true||Number(report.brandEvidencePages)!==136||Number(report.brandEvidenceMetricRows)!==544||Number(report.brandEvidenceCostPartRows)!==544||Number(report.brandEvidenceFlowRows)!==136||Number(report.brandEvidenceMissingSalesRows)!==2||Number(report.brandEvidenceMissingAreaRows)!==3||report.brandEvidenceCandidateOnly!==true)err.push(`report brandEvidence ${report.brandEvidenceUx}/${report.brandEvidencePages}/${report.brandEvidenceMetricRows}/${report.brandEvidenceCostPartRows}/${report.brandEvidenceFlowRows}/${report.brandEvidenceMissingSalesRows}/${report.brandEvidenceMissingAreaRows}/${report.brandEvidenceCandidateOnly}`);
...
try { const retention=validateRetentionWorkspace(out); if(retention.brandWorkspaces!==136||retention.updatesRadarRows!==12||retention.localOnlyPersistence!==true||retention.candidateNotes!==true||retention.checklistProgressSummary!==true||retention.candidatePlanning!==true||retention.shortlistDashboard!==true||retention.shortlistBackup!==true||retention.changeInbox!==true||retention.changeInboxSurfaces!==2||retention.changeAcknowledgement!==true||retention.changedOnlyFilter!==true||retention.shortlistDecisionBoard!==true||retention.shortlistCsvExport!==true||retention.editorialRails!==3||retention.editorialGuideLinks!==9)err.push(`retention workspace coverage ${retention.brandWorkspaces}/${retention.updatesRadarRows}/${retention.localOnlyPersistence}/${retention.candidateNotes}/${retention.checklistProgressSummary}/${retention.candidatePlanning}/${retention.shortlistDashboard}/${retention.shortlistBackup}/${retention.changeInbox}/${retention.changeInboxSurfaces}/${retention.changeAcknowledgement}/${retention.changedOnlyFilter}/${retention.shortlistDecisionBoard}/${retention.shortlistCsvExport}/${retention.editorialRails}/${retention.editorialGuideLinks}`); } catch(error) { err.push(`retention workspace: ${error.message}`); }
...
try { const context=validateContextualGuides(out); if(context.brandGuideRails!==136||context.categoryGuideRails!==16||context.hubGuideRails!==6||context.totalGuideRails!==158||context.guideLinks!==474||context.candidateOnly!==true)err.push(`contextual guides coverage ${context.brandGuideRails}/${context.categoryGuideRails}/${context.hubGuideRails}/${context.totalGuideRails}/${context.guideLinks}/${context.candidateOnly}`); } catch(error) { err.push(`contextual guides: ${error.message}`); }
try { const evidence=validateBrandEvidence(out); if(evidence.pages!==136||evidence.metricRows!==544||evidence.costPartRows!==544||evidence.flowRows!==136||evidence.missingSalesRows!==2||evidence.missingAreaRows!==3||evidence.candidateOnly!==true)err.push(`brand evidence coverage ${evidence.pages}/${evidence.metricRows}/${evidence.costPartRows}/${evidence.flowRows}/${evidence.missingSalesRows}/${evidence.missingAreaRows}/${evidence.candidateOnly}`); } catch(error) { err.push(`brand evidence: ${error.message}`); }

if(err.length){console.error(JSON.stringify({v11_52ReleaseCandidateValidation:'FAIL',count:err.length,bodyCoverage,noindex,singleH1,metaDescriptions,canonicalPreview,compareSelectedOptions:selectedOptions.length,reportSummary:{brandLowerFunnelUx:report.brandLowerFunnelUx,brandDecisionPages:report.brandDecisionPages,brandDecisionTagged:report.brandDecisionTagged,brandLowerFunnelEligiblePages:report.brandLowerFunnelEligiblePages,broken:report.brokenInternalLinks?.length,missingAssets:report.missingAssets?.length,candidateIssues:report.candidateIssues?.length,titleDup:report.titleDuplicateGroups?.length,descDup:report.descriptionDuplicateGroups?.length,h1Dup:report.h1DuplicateGroups?.length,canonicalDup:report.canonicalDuplicateGroups?.length,imageMissingAlt:report.imageMissingAlt,compareHydrationAligned:report.compareHydrationAligned,compareDecisionUx:report.compareDecisionUx,staticCompareDecisionUx:report.staticCompareDecisionUx,staticCompareDecisionPages:report.staticCompareDecisionPages,legacyCompareDecisionUx:report.legacyCompareDecisionUx,legacyCompareDecisionPages:report.legacyCompareDecisionPages,toolsDecisionUx:report.toolsDecisionUx,homeDecisionUx:report.homeDecisionUx,trustConsistencyUx:report.trustConsistencyUx,discoveryHubsUx:report.discoveryHubsUx,visualIntegrityUx:report.visualIntegrityUx,visualIntegrityRemovedStockImages:report.visualIntegrityRemovedStockImages,visualIntegrityAffectedPages:report.visualIntegrityAffectedPages,prelaunchQualityUx:report.prelaunchQualityUx,rankingFaqItems:report.rankingFaqItems,historyNotePages:report.historyNotePages,historyNotes:report.historyNotes,brandAnchorOffsets:report.brandAnchorOffsets,releaseInputsOwnedByProductionGate:report.releaseInputsOwnedByProductionGate,retentionWorkspaceUx:report.retentionWorkspaceUx,retentionBrandWorkspaces:report.retentionBrandWorkspaces,updatesRadarRows:report.updatesRadarRows,localOnlyPersistence:report.localOnlyPersistence,candidateNotes:report.candidateNotes,checklistProgressSummary:report.checklistProgressSummary,candidatePlanning:report.candidatePlanning,shortlistDashboard:report.shortlistDashboard,shortlistBackup:report.shortlistBackup,changeInbox:report.changeInbox,changeInboxSurfaces:report.changeInboxSurfaces,changeAcknowledgement:report.changeAcknowledgement,changedOnlyFilter:report.changedOnlyFilter,shortlistDecisionBoard:report.shortlistDecisionBoard,shortlistCsvExport:report.shortlistCsvExport,editorialRails:report.editorialRails,editorialGuideLinks:report.editorialGuideLinks,publisherValueUx:report.publisherValueUx,publisherValueItems:report.publisherValueItems,automationDisclosure:report.automationDisclosure,originalAnalysisDisclosure:report.originalAnalysisDisclosure,contextualGuidesUx:report.contextualGuidesUx,contextualBrandRails:report.contextualBrandRails,contextualCategoryRails:report.contextualCategoryRails,contextualHubRails:report.contextualHubRails,contextualTotalRails:report.contextualTotalRails,contextualGuideLinks:report.contextualGuideLinks,contextualCandidateOnly:report.contextualCandidateOnly,brandEvidenceUx:report.brandEvidenceUx,brandEvidencePages:report.brandEvidencePages,brandEvidenceMetricRows:report.brandEvidenceMetricRows,brandEvidenceCostPartRows:report.brandEvidenceCostPartRows,brandEvidenceFlowRows:report.brandEvidenceFlowRows,brandEvidenceMissingSalesRows:report.brandEvidenceMissingSalesRows,brandEvidenceMissingAreaRows:report.brandEvidenceMissingAreaRows,brandEvidenceCandidateOnly:report.brandEvidenceCandidateOnly,rcReady:report.rcReady},errors:err.slice(0,220)},null,2));process.exit(1)}
console.log(JSON.stringify({v11_52ReleaseCandidateValidation:'PASS',htmlPages:311,bodyCoverage,candidates:184,noindex,singleH1,metaDescriptions,canonicalPreview,brandDecisionPages,brandDecisionTagged,brandLowerFunnelEligiblePages,brandLowerFunnelUx:true,compareSelectedOptions:2,compareHydrationAligned:true,compareDecisionUx:true,staticCompareDecisionPages:7,staticCompareDecisionUx:true,legacyCompareDecisionPages:2,legacyCompareDecisionUx:true,toolsDecisionUx:true,homeDecisionUx:true,trustConsistencyUx:true,discoveryHubsUx:true,visualIntegrityUx:true,prelaunchQualityUx:true,rankingFaqItems:4,historyNotePages:report.historyNotePages,brandAnchorOffsets:true,releaseInputsOwnedByProductionGate:true,retentionWorkspaceUx:true,retentionBrandWorkspaces:136,updatesRadarRows:12,localOnlyPersistence:true,candidateNotes:true,checklistProgressSummary:true,candidatePlanning:true,shortlistDashboard:true,shortlistBackup:true,changeInbox:true,changeInboxSurfaces:2,changeAcknowledgement:true,changedOnlyFilter:true,shortlistDecisionBoard:true,shortlistCsvExport:true,editorialRails:3,editorialGuideLinks:9,publisherValueUx:true,publisherValueItems:6,automationDisclosure:true,originalAnalysisDisclosure:true,contextualGuidesUx:true,contextualBrandRails:136,contextualCategoryRails:16,contextualHubRails:6,contextualTotalRails:158,contextualGuideLinks:474,contextualCandidateOnly:true,brandEvidenceUx:true,brandEvidencePages:136,brandEvidenceMetricRows:544,brandEvidenceCostPartRows:544,brandEvidenceFlowRows:136,brandEvidenceMissingSalesRows:2,brandEvidenceMissingAreaRows:3,brandEvidenceCandidateOnly:true,internalLinksChecked:report.totalInternalLinks,assetsChecked:report.totalInternalAssets,imagesChecked:report.imageCount,rcReady:true,productionDeployed:false},null,2));
