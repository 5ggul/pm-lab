...
if(report.retentionWorkspaceUx!==true||Number(report.retentionBrandWorkspaces)!==136||Number(report.updatesRadarRows)!==12||report.localOnlyPersistence!==true||report.candidateNotes!==true||report.checklistProgressSummary!==true||Number(report.editorialRails)!==3||Number(report.editorialGuideLinks)!==9)err.push(`report retention ${report.retentionWorkspaceUx}/${report.retentionBrandWorkspaces}/${report.updatesRadarRows}/${report.localOnlyPersistence}/${report.candidateNotes}/${report.checklistProgressSummary}/${report.editorialRails}/${report.editorialGuideLinks}`);
...
try { const retention=validateRetentionWorkspace(out); if(retention.brandWorkspaces!==136||retention.updatesRadarRows!==12||retention.localOnlyPersistence!==true||retention.candidateNotes!==true||retention.checklistProgressSummary!==true||retention.editorialRails!==3||retention.editorialGuideLinks!==9)err.push(`retention workspace coverage ${retention.brandWorkspaces}/${retention.updatesRadarRows}/${retention.localOnlyPersistence}/${retention.candidateNotes}/${retention.checklistProgressSummary}/${retention.editorialRails}/${retention.editorialGuideLinks}`); } catch(error) { err.push(`retention workspace: ${error.message}`); }
