...
if(report.retentionWorkspaceUx!==true||Number(report.retentionBrandWorkspaces)!==136||Number(report.updatesRadarRows)!==12||report.localOnlyPersistence!==true)err.push(`report retention ${report.retentionWorkspaceUx}/${report.retentionBrandWorkspaces}/${report.updatesRadarRows}/${report.localOnlyPersistence}`);
...
try { const retention=validateRetentionWorkspace(out); if(retention.brandWorkspaces!==136||retention.updatesRadarRows!==12||retention.localOnlyPersistence!==true)err.push(`retention workspace coverage ${retention.brandWorkspaces}/${retention.updatesRadarRows}/${retention.localOnlyPersistence}`); } catch(error) { err.push(`retention workspace: ${error.message}`); }
