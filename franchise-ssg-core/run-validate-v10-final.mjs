...
for(const f of files){
  const h=await fs.readFile(f,'utf8'),rel=path.relative(out,f);
  const css=[...h.matchAll(/<link rel="stylesheet" href="([^"]+)"/g)].map(x=>x[1]);
  if(css.length!==1||!css[0].endsWith('/assets/site.css'))badCss.push(`${rel}:${css.join(',')}`);
  if(PREVIEW&&!h.includes('noindex,nofollow,noarchive,nosnippet'))badRobots.push(rel);
  const t=h.match(/<title>([^<]+)<\/title>/)?.[1];if(t){if(titles.has(t))throw new Error(`Duplicate title: ${t} (${titles.get(t)}, ${rel})`);titles.set(t,rel)}
  const h1=(h.match(/<h1(?:\s|>)/g)||[]).length;if(h1>1)multipleH1.push(`${rel}:${h1}`);
  const body=h.split('<body>')[1]||h;if(/>\s*(?:undefined|NaN|null)\s*</.test(body))badVisible.push(rel);
  for(const m of h.matchAll(/href="([^"]+)"/g)){let href=m[1];if(!href.startsWith(BASE))continue;href=href.slice(BASE.length).split('#')[0].split('?')[0];try{href=decodeURIComponent(href)}catch{}const target=href===''||href==='/'?path.join(out,'index.html'):href.endsWith('/')?path.join(out,href.replace(/^\//,''),'index.html'):path.join(out,href.replace(/^\//,''));try{await fs.stat(target)}catch{broken.push(`${rel} -> ${m[1]}`)}}
}
...
for(const old of ['v7.css','v8.css','v9.css']){let exists=true;try{await fs.stat(path.join(out,'assets',old))}catch{exists=false}if(exists)throw new Error(`Legacy CSS output still exists: ${old}`)}
...
for(const rel of ['tools/startup-cost','tools/monthly-profit-simulator']){const h=await read(rel);for(const m of ['data-v10-calculator-page','data-copy-url','계산','입력값'])if(!h.includes(m))throw new Error(`${rel} missing ${m}`)}
...
const sitemap=await fs.readFile(path.join(out,'sitemap.xml'),'utf8');if(PREVIEW&&sitemap.includes('<url>'))throw new Error('Preview sitemap must not expose noindex URLs');
const robotsTxt=await fs.readFile(path.join(out,'robots.txt'),'utf8');if(PREVIEW&&!robotsTxt.includes('Disallow: /'))throw new Error('Preview robots.txt must disallow all');
console.log(JSON.stringify({v10Validation:'PASS',htmlPages:files.length,uniqueTitles:titles.size,brokenInternalLinks:0,brandPages,candidates,directoryRows,officialRecords:quality.official.records,matched:quality.official.matched,unmatched:quality.official.unmatched,ambiguous:quality.official.ambiguous,legacyCssLinks:0,previewNoindex:PREVIEW},null,2));