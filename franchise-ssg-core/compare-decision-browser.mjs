...
async function scanEvidence(page,width){
  ...
  return{costCards:await page.locator('[data-v52-compare-section="cost"] .v34-ring-card').count(),benchmarkReferences:await refs.count(),stickyDiff:width<=760,verifiedCards:await verifiedRows.count(),directoryLinks:await directoryLinks.count()};
}
