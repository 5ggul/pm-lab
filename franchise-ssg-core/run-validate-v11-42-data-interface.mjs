...
const report=JSON.parse(await fs.readFile(path.join(out,'v11-42-data-interface.json'),'utf8'));
...
if(report.genericScenesRemoved<150||report.orbitsRemoved<300||report.photoBreaksRemoved<1||report.insetPhotosRemoved<1||report.parallaxAttrsRemoved<150)err.push(`cleanup counts ${JSON.stringify({generic:report.genericScenesRemoved,orbit:report.orbitsRemoved,breaks:report.photoBreaksRemoved,inset:report.insetPhotosRemoved,parallax:report.parallaxAttrsRemoved})}`);
...
if(err.length){console.error(JSON.stringify({v11_42DataInterfaceValidation:'FAIL',count:err.length,htmlPages:htmlFiles.length,classCount,scriptCount,brands,brandHero,categories,categoryScene,noindex,errors:err.slice(0,220)},null,2));process.exit(1)}
console.log(JSON.stringify({v11_42DataInterfaceValidation:'PASS',htmlPages:htmlFiles.length,classCount,scriptCount,brands,brandHero,categories,categoryScene,candidates:184,noindex,cleanup:{genericScenesRemoved:report.genericScenesRemoved,orbitsRemoved:report.orbitsRemoved,photoBreaksRemoved:report.photoBreaksRemoved,insetPhotosRemoved:report.insetPhotosRemoved,parallaxAttrsRemoved:report.parallaxAttrsRemoved},productionDeployed:false},null,2));
