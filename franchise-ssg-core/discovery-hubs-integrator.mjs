...
const BRANDS_FAQ=`${marks.brandsFaq[0]}<section class="v52-discovery-faq" data-v52-discovery-faq="brands" aria-labelledby="v52-brands-faq-title"><div class="v52-discovery-head"><span>기준 설명</span><h2 id="v52-brands-faq-title">브랜드 목록을 볼 때 자주 묻는 기준</h2></div><div class="v52-discovery-faq-list">${BRANDS_FAQS.map(x=>faq(...x)).join('')}</div></section>${marks.brandsFaq[1]}`;

const CATEGORIES_RAIL=`${marks.categoriesRail[0]}<section class="v52-discovery-rail" data-v52-discovery-rail="categories" aria-labelledby="v52-categories-next"><div class="v52-discovery-head"><span>업종 탐색</span><h2 id="v52-categories-next">업종을 비교한 뒤 후보로 좁히기</h2></div><div class="v52-discovery-cards">${card('20개 업종','업종 기준선 비교','각 업종의 공식 매칭 수·분석 표본·공개 창업비용 중앙값을 먼저 봅니다.','/categories/','#').replace(' href="'+BASE+'/categories/" #',' href="#v52-category-grid"')}${card('예산도 함께','예산×업종 교차 확인','내 예산 이하 후보가 업종별로 몇 개인지 바로 좁힙니다.','/explore/')}${card('브랜드 찾기','업종으로 브랜드 필터','브랜드 목록에서 업종·비용·가맹점·점포 변화를 함께 필터링합니다.','/brands/')}${card('후보 비교','최대 4개 비교','업종을 본 뒤 남은 후보를 같은 지표 기준으로 나란히 확인합니다.','/compare/')}</div></section>${marks.categoriesRail[1]}`;
...
const CATEGORIES_FAQ=`${marks.categoriesFaq[0]}<section class="v52-discovery-faq" data-v52-discovery-faq="categories" aria-labelledby="v52-categories-faq-title"><div class="v52-discovery-head"><span>기준 설명</span><h2 id="v52-categories-faq-title">업종 중앙값을 해석할 때</h2></div><div class="v52-discovery-faq-list">${CATEGORIES_FAQS.map(x=>faq(...x)).join('')}</div></section>${marks.categoriesFaq[1]}`;

const EXPLORE_RAIL=`${marks.exploreRail[0]}<section class="v52-discovery-rail" data-v52-discovery-rail="explore" aria-labelledby="v52-explore-next"><div class="v52-discovery-head"><span>탐색 순서</span><h2 id="v52-explore-next">예산·업종 조건을 다음 단계로 이어가기</h2></div><div class="v52-discovery-cards">${card('조건 설정','예산·업종 좁히기','이 화면에서 공개비용·업종·가맹점·평균매출 공개지표 조건을 먼저 좁힙니다.','/explore/','#').replace(' href="'+BASE+'/explore/" #',' href="#finder"')}${card('브랜드 확인','브랜드 목록에서 이어보기','현재 예산·업종 조건을 브랜드 필터로 넘겨 이름과 점포 변화까지 확인합니다.','/brands/','data-v52-directory-handoff')}${card('후보 비교','후보끼리 비교','남은 후보를 최대 4개까지 같은 공개지표로 비교합니다.','/compare/')}${card('자금 계산','총 준비자금 계산','공개비용 외 임대·권리금·추가공사·운전자금을 별도로 더합니다.','/tools/startup-cost/')}</div><p class="v52-discovery-note" data-v52-handoff-note>브랜드 목록 인계는 선택한 예산 프리셋과 업종을 이어갑니다.</p></section>${marks.exploreRail[1]}`;
...
const EXPLORE_FAQ=`${marks.exploreFaq[0]}<section class="v52-discovery-faq" data-v52-discovery-faq="explore" aria-labelledby="v52-explore-faq-title"><div class="v52-discovery-head"><span>기준 설명</span><h2 id="v52-explore-faq-title">예산별 찾기를 해석할 때</h2></div><div class="v52-discovery-faq-list">${EXPLORE_FAQS.map(x=>faq(...x)).join('')}</div></section>${marks.exploreFaq[1]}`;
