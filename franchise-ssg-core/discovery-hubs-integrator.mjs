...
function addAfter(text,needle,block,label){if(!text.includes(needle))throw new Error(`${label} insertion point missing`);return text.replace(needle,needle+block);}
...
function applyCategories(root){const file=path.join(root,'categories/index.html');let html=fs.readFileSync(file,'utf8'),before=html;if(!html.includes('<h1>업종 데이터</h1>')||!html.includes('<div class="report-grid">'))throw new Error('Categories hub shape mismatch');if(!html.includes('id="v52-category-grid"'))html=html.replace('<div class="report-grid">','<div class="report-grid" id="v52-category-grid" data-v52-category-grid="1">');
  let next=replaceMarked(html,marks.categoriesRail,CATEGORIES_RAIL);html=next??addBefore(html,'<div class="report-grid" id="v52-category-grid" data-v52-category-grid="1">',CATEGORIES_RAIL,'categories rail');
