...
- 지역 상세는 전체 업소·외식 업소·면적·전체 업소 밀도·업종별 업소 수·업종별 면적 대비 밀도를 같은 구조로 보여줍니다.
- `/areas/compare/`에서 두 지역과 한 업종을 직접 골라 절대 업소 수와 ㎢당 공급량을 비교합니다.
- 지역 밀도는 공급 관찰값일 뿐 수요·매출·성공 가능성·추천 점수로 사용하지 않습니다.
...
기본 스크립트는 `run-generate-v6-final.mjs` → `run-validate-v6.mjs` 순서로 실행합니다.
