...
if(!Number.isFinite(uiMinor)||uiMinor<37||manifest.v11_37?.averageSalesPositiveOnly!==true)err.push(`manifest v11.37 semantics under ${manifest.uiVersion}`);
...
for(const [slug,c] of Object.entries(snap.categories||{})){
  const rows=snap.brands.filter(b=>b.categorySlug===slug),positives=rows.filter(b=>positive(b.sales));
...
for(const b of snap.brands){
  const h=await fs.readFile(fileFor(b.route),'utf8'),c=snap.categories[b.categorySlug];
  if(!positive(b.sales)){
    ...
    const pct=Number(b.category.salesPercentile).toFixed(0),expected=`${b.category.salesRank}/${c.sales.count} · ${pct}%`;
    if(!h.includes(expected))err.push(`sales denominator ${b.slug} expected ${expected}`);
  }
  const dm=h.match(datasetRe);if(!dm){err.push(`dataset jsonld ${b.slug}`);continue}
  let data;try{data=JSON.parse(dm[1])}catch(e){err.push(`dataset parse ${b.slug}: ${e.message}`);continue}
  const measured=Array.isArray(data.variableMeasured)?data.variableMeasured:[],salesMetric=measured.find(x=>String(x?.name||'')==='평균매출 공개지표'),rankMetric=measured.find(x=>String(x?.name||'').includes('평균매출 공개지표 높은 순 위치'));
  if(!positive(b.sales)){if(salesMetric)err.push(`structured zero/missing sales leaked ${b.slug}`);if(rankMetric)err.push(`structured missing sales rank leaked ${b.slug}`)}
  else{if(!salesMetric||Number(salesMetric.value)!==Number(b.sales))err.push(`structured sales value ${b.slug}`);if(finite(b.category?.salesRank)&&c?.sales?.count){if(!rankMetric||Number(rankMetric.value)!==Number(b.category.salesRank))err.push(`structured sales rank ${b.slug}`);if(rankMetric?.unitText!==`${c.sales.count}개 공개값 중 순위`)err.push(`structured sales denominator ${b.slug}`)}}
}
...
if(err.length){console.error(JSON.stringify({v11_37Validation:'FAIL',currentUiVersion:manifest.uiVersion,count:err.length,errors:err.slice(0,160)},null,2));process.exit(1)}
console.log(JSON.stringify({v11_37Validation:'PASS',currentUiVersion:manifest.uiVersion,brands:snap.brand_count,categories:snap.category_count,candidates:candidates.length,unavailableAverageSales:unavailable.length,denominatorFixes:report.denominatorFixes,structuredDataFixes:report.structuredDataFixes},null,2));
