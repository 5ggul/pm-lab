...
export function validateRetentionWorkspace(root){
 ...
 for(const b of snapshot.brands){
   ...
   for(const k of ['data-v52-save-brand','data-v52-check="disclosure"','data-v52-check="simulation"','data-v52-candidate-note','data-v52-note-count'])if(!html.includes(k))throw new Error(`Brand retention control missing ${k} ${b.route}`);
 ...
 for(const token of ['franchiseLabShortlistV1','franchiseLabRecentV1','franchiseLabChecklistV1:','franchiseLabNoteV1:','data-v52-load-saved','data-v52-ack-change','data-v52-candidate-note'])if(!js.includes(token))throw new Error(`Retention JS missing ${token}`);
 return{retentionWorkspace:true,brandWorkspaces,assetPages,homeWorkspace:true,compareSavedLoader:true,updatesRadarRows:12,localOnlyPersistence:true,candidateNotes:true,checklistProgressSummary:true,editorialRails:3,editorialGuideLinks};
}
