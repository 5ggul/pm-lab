...
function compareBlock(snapshot){
 return \`${COMPARE_START}<section class="v52-saved-compare" data-v52-saved-compare="1" aria-labelledby="v52-saved-compare-title"><div class="v52-retention-head"><div><small>저장 후보</small><strong id="v52-saved-compare-title">내 후보로 바로 비교</strong></div><p>브랜드 상세에서 저장한 후보를 최대 4개까지 현재 비교 화면에 불러옵니다.</p></div><div class="v52-saved-compare-list" data-v52-saved-compare-list><span class="v52-retention-empty">저장한 후보가 없습니다.</span></div><div class="v52-saved-compare-actions"><button type="button" class="v52-load-saved" data-v52-load-saved>후보 2개 이상 저장하면 불러올 수 있습니다</button><span>저장 정보는 이 브라우저에만 남습니다.</span></div><section class="v52-decision-board" data-v52-decision-board aria-labelledby="v52-decision-board-title"><div class="v52-decision-board-head"><div><small>저장 후보 작업표</small><strong id="v52-decision-board-title">현재값과 확인 진행상태</strong></div><button type="button" data-v52-export-decision-csv>CSV 내보내기</button></div><div class="v52-decision-board-labels" aria-hidden="true"><span>브랜드</span><span>상태</span><span>확인</span><span>공개비용</span><span>가맹점</span><span>연평균매출</span><span>점포변화</span><span>저장 후 변화</span></div><div class="v52-decision-board-rows" data-v52-decision-board-rows><p class="v52-retention-empty">저장한 후보가 없습니다.</p></div><p class="v52-local-note">수치는 현재 trusted 공개 스냅샷 기준이며, 상태·메모·다음 행동은 현재 브라우저에만 저장됩니다. 이 작업표는 브랜드 추천이나 순위를 만들지 않습니다.</p></section>${datasetScript(snapshot)}${editorialRail('compare')}<\/section>${COMPARE_END}\`;
}
...
export function validateRetentionWorkspace(root){
 ...
 for(const token of ['franchiseLabShortlistV1','franchiseLabRecentV1','franchiseLabChecklistV1:','franchiseLabNoteV1:','franchiseLabPlanV1:','data-v52-load-saved','data-v52-ack-change','data-v52-candidate-note','data-v52-export-shortlist','data-v52-import-shortlist','data-v52-change-inbox','data-v52-ack-saved-change','data-v52-ack-all-changes','data-v52-change-only','data-v52-decision-board','data-v52-export-decision-csv'])if(!js.includes(token))throw new Error(`Retention JS missing ${token}`);
 return{retentionWorkspace:true,brandWorkspaces,assetPages,homeWorkspace:true,compareSavedLoader:true,shortlistDecisionBoard:true,shortlistCsvExport:true,updatesRadarRows:12,localOnlyPersistence:true,candidateNotes:true,checklistProgressSummary:true,candidatePlanning:true,shortlistDashboard:true,shortlistBackup:true,changeInbox:true,changeInboxSurfaces:2,changeAcknowledgement:true,changedOnlyFilter:true,editorialRails:3,editorialGuideLinks};
}
