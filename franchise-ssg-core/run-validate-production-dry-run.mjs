...
if(report.inputContract!=='STRICT_EXACT_HTTPS_ORIGIN; NO_PATH_QUERY_HASH_CREDENTIALS_OR_RESERVED_HOST; FINAL_LEGAL_SOURCES; LOCKED_RELEASE_POLICY')errors.push('strict release-input contract marker changed');
...
if(report.config?.usingExample===true){
  ...
  if(!Array.isArray(report.config?.blockers)||report.config.blockers.length<8)errors.push('example config strict blockers missing');
}
if(report.releasePolicy?.indexOnlyProductionCandidates?.ok!==true)errors.push('indexOnlyProductionCandidates policy drift');
if(report.releasePolicy?.keepNonCandidatesNoindex?.ok!==true)errors.push('keepNonCandidatesNoindex policy drift');
if(report.releasePolicy?.requireManualApprovalBeforeDeploy?.ok!==true)errors.push('requireManualApprovalBeforeDeploy policy drift');
if(report.releasePolicy?.deployFromDryRun?.ok!==true)errors.push('deployFromDryRun policy drift');
...
console.log(JSON.stringify({productionDryRunValidation:'PASS',decision:report.decision,candidates:candidates.length,usingExample:report.config?.usingExample,missing:report.config?.missing?.length||0,strictInputContract:true,previewUnchanged:true,deployAttempted:false},null,2));
