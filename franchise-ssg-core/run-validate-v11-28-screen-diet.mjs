...
if (report.uiVersion !== '11.28' || report.schemaVersion !== 2) fail('v11.28 report missing or stale');
...
if (report.categorySummariesRemoved !== report.categoryRangeSectionsFound) fail(`category range summaries mismatch: ${report.categorySummariesRemoved}/${report.categoryRangeSectionsFound}`);
...
for (const slug of Object.keys(snapshot.categories)) {
  ...
  for (const [id, compactHeading] of [['distribution', '<h2>비용분포</h2>'], ['range', '<h2>업종범위</h2>']]) {
    ...
    if (id === 'distribution') checkedCategoryDistribution += 1;
    else checkedCategoryRange += 1;
  }
}
