...
const awkward=/\b(?:낮고|높고|적고|많고) \([-+]?\d+(?:\.\d+)?%\)\./;
let filesPatched=0,phrasesFixed=0,residualAwkward=0;
for(const b of snapshot.brands||[]){
  ...
  if(awkward.test(block))residualAwkward++;
...
if(residualAwkward)throw new Error(`v11.48 copy polish residual awkward phrases in ${residualAwkward} brand pages`);
report.copyPolishApplied=true;report.copyPolishFiles=filesPatched;report.copyPolishPhrases=phrasesFixed;report.copyPolishResidualAwkward=0;
await fs.writeFile(reportPath,JSON.stringify(report,null,2)+'\n','utf8');
console.log(JSON.stringify({v11_48CopyPolish:'PASS',filesPatched,phrasesFixed,residualAwkward:0},null,2));
