...
function fixture(){const root=fs.mkdtempSync(path.join(os.tmpdir(),'discovery-hubs-'));for(const d of ['brands','categories','explore','assets'])fs.mkdirSync(path.join(root,d),{recursive:true});fs.writeFileSync(path.join(root,'assets/discovery-hubs.css'),'/* test */');fs.writeFileSync(path.join(root,'assets/discovery-hubs.js'),'// test');
 ...
 fs.writeFileSync(path.join(root,'explore/index.html'),`<!doctype html><html>${head('explore',dataset)}<body><main id="main" data-v11-budget-explorer="1" data-v29-budget-category="1"><div class="shell page"><h1>예산별 프랜차이즈 찾기</h1><section class="block" id="budget-summary"></section><section id="finder"><form data-budget-form><input name="budget" value="10000"><select name="cat"><option value="">전체</option><option value="cafe">카페</option></select></form></section></div></main></body></html>`);return root;}

test('unit: injects three task rails, FAQs, cost parity and structured data',()=>{const root=fixture();const result=applyDiscoveryHubs(root);assert.equal(result.changed,true);const brands=fs.readFileSync(path.join(root,'brands/index.html'),'utf8'),categories=fs.readFileSync(path.join(root,'categories/index.html'),'utf8'),explore=fs.readFileSync(path.join(root,'explore/index.html'),'utf8');assert.ok(brands.includes('<option value="7000">7,000만원</option>'));assert.equal((brands.match(/data-v52-directory-empty/g)||[]).length,1);assert.equal((brands.match(/class="v52-discovery-card"/g)||[]).length,4);assert.equal((categories.match(/class="v52-discovery-card"/g)||[]).length,4);assert.equal((explore.match(/class="v52-discovery-card"/g)||[]).length,4);assert.equal((brands.match(/<details>/g)||[]).length,4);assert.equal((categories.match(/<details>/g)||[]).length,4);assert.equal((explore.match(/<details>/g)||[]).length,4);const item=JSON.parse(categories.match(/data-v52-discovery-categories-list>([\s\S]*?)<\/script>/)[1]);assert.equal(item['@type'],'ItemList');assert.equal(item.itemListElement.length,20);assert.equal(validateDiscoveryHubs(root).categoryItemList,20);});
