...
for(const token of ['data-v49-compare-live','data-v49-startup-summary','data-v49-startup-reset','data-v49-remove'])if(!js.includes(token))err.push(`js ${token}`);

let brandBlocks=0,brandRows=0,noindex=0,v48Preserved=0,bodyFlag=0;
for(const b of snap.brands||[]){
  ...
  if(block[1].includes('실제 부담 없음으로 단정하지 말고')&&!(Object.values(b.components||{}).some(v=>Number(v)===0)))err.push(`zero warning mismatch ${b.slug}`);
...
if(brandBlocks!==136||brandRows!==408||v48Preserved!==136||bodyFlag!==136)err.push(`brand coverage ${brandBlocks}/${brandRows}/${v48Preserved}/${bodyFlag}`);
...
const six=await fs.readFile(fileFor('/brands/666버거/'),'utf8');
if(!six.includes('0원 공개항목'))err.push('666 zero warning');
...
if(err.length){console.error(JSON.stringify({v11_49BulkUsabilityValidation:'FAIL',count:err.length,brandBlocks,brandRows,v48Preserved,bodyFlag,noindex,errors:err.slice(0,200)},null,2));process.exit(1)}
console.log(JSON.stringify({v11_49BulkUsabilityValidation:'PASS',brands:136,brandRows:408,v48Preserved,noindex,candidates:184,compareHub:true,startupTool:true,mobile:true,productionDeployed:false},null,2));
