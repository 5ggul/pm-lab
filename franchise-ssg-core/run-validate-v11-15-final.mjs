...
const readTool=slug=>fs.readFile(path.join(out,`tools/${slug}/index.html`),'utf8');
const filter=await readTool('brand-filter');
const categoryMedian=await readTool('category-median');
const fixed=await readTool('monthly-fixed-cost');
const breakEven=await readTool('break-even');
const openClose=await readTool('open-close-rate');
const density=await readTool('store-density');
...
const approved=(report.approvedToolRoutes||[]).map(normalizeRoute),blocked=(report.blockedToolRoutes||[]).map(normalizeRoute);
...
if(report.parsedBrandCards<150)errors.push(`brand-card coverage too low: ${report.parsedBrandCards}`);
...
if(report.brandFilterMissingCostCards<1)errors.push('missing cost repair did not run');
if(report.brandFilterMissingStoreCards<1)errors.push('missing store repair did not run');
...
for(const route of approved)if(!candidates.has(route))errors.push(`approved tool missing from candidates: ${route}`);
for(const route of blocked)if(candidates.has(route))errors.push(`blocked tool leaked into candidates: ${route}`);
...
if(parseNullable('')!==null||parseNullable('   ')!==null||parseNullable(null)!==null)errors.push('nullable parser treats blank as a number');
if(parseNullable('0')!==0)errors.push('explicit zero is not preserved');
if(passesMaxCost('',10000)!==false)errors.push('missing cost still passes a max-cost filter');
if(passesMaxCost('9000',10000)!==true)errors.push('valid cost fails max-cost semantics');

if(!categoryMedian.includes('data-tool="category-median-v11"')||!categoryMedian.includes('data-v11-15-real-tool="1"'))errors.push('category-median interactive form missing');
if(categoryMedian.includes('<div class="tool-static">'))errors.push('category-median remains static');
const optionCount=(categoryMedian.match(/<option value="[^"]*\/brands\//g)||[]).length;
if(optionCount!==report.categoryMedianOptions)errors.push(`category-median option mismatch ${optionCount}/${report.categoryMedianOptions}`);
for(const marker of ['data-cm-cost','data-cm-median','data-cm-diff','data-cm-sample','data-cm-brand-link'])if(!categoryMedian.includes(marker))errors.push(`category-median marker missing: ${marker}`);
if(!categoryMedian.includes('공정위 가맹사업거래 비용 공개데이터'))errors.push('category-median official source link missing');

if(!fixed.includes('data-tool="monthly-fixed-cost-v11"')||!fixed.includes('data-v11-15-real-tool="1"'))errors.push('monthly-fixed-cost interactive form missing');
if(fixed.includes('<div class="tool-static">'))errors.push('monthly-fixed-cost remains static');
...
if(!fixed.includes('type="number"'))errors.push('monthly-fixed-cost numeric inputs missing');
...
if(!density.includes('<div class="tool-static">'))errors.push('store-density changed before real local data exists');
if(!app.includes('/* v11.15 tool trust */'))errors.push('v11.15 app marker missing');
...
for(const [name,html] of [['brand-filter',filter],['category-median',categoryMedian],['monthly-fixed-cost',fixed],['break-even',breakEven],['open-close-rate',openClose],['store-density',density]])if(!html.includes('noindex,nofollow,noarchive,nosnippet'))errors.push(`${name} preview noindex policy lost`);
...
if(errors.length){console.error(JSON.stringify({v11_15Validation:'FAIL',errors,summary:{approvedTools:approved.length,blockedTools:blocked.length,categoryMedianOptions:report.categoryMedianOptions,missingCostCards:missingCost,missingStoreCards:missingStores}},null,2));process.exit(1)}
