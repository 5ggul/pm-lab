...
if(toolRows.length!==approvedToolRoutes.length)throw new Error(`v11.20 expected ${approvedToolRoutes.length} verified tool rows, parsed ${toolRows.length}`);
if(!candidateSet.has('/explore/'))throw new Error('v11.20 requires the v11.19 /explore/ production candidate');

const journeyRows=[
  {href:`${BASE}/explore/`,name:'예산별 찾기',desc:`신뢰 게이트를 통과한 ${budgetIntent.trustedBrands}개 브랜드를 예산·업종·가맹점 조건으로 좁힙니다.`},
...
const journeySection=`<section class="home-section home-paths" data-v11-20-home-paths="1"><div class="shell"><div class="section-head"><h2>원하는 방식으로 찾기</h2></div><div class="authority-directory">${journeyRows.map(row=>`<a href="${row.href}"><strong>${esc(row.name)}</strong><span>${esc(row.desc)}</span></a>`).join('')}</div></div></section>`;
const toolSection=`<section class="home-section home-tool-directory" data-v11-20-home-tools="1"><div class="shell"><div class="section-head"><h2>계산·분석 도구 ${toolRows.length}개</h2><a href="${BASE}/tools/">도구 전체 보기</a></div><div class="authority-directory">${toolRows.map(row=>`<a href="${row.href}"><strong>${esc(row.name)}</strong><span>${esc(row.goal)}</span></a>`).join('')}</div><p class="data-note">공식 공개값, 사용자가 입력한 가정, 공개값에서 계산한 파생 통계를 도구별로 구분합니다.</p></div></section>`;

home=home.replace('<div><b>3</b><span>계산기</span></div>',`<div><b>${toolRows.length}</b><span>검증 도구</span></div>`);
if(!home.includes('data-v11-20-home-paths="1"')){
...
quality.contentTrust={...(quality.contentTrust||{}),version:'11.20',generatedAt,internalAuthorityPolicy:true,budgetExplorerPreserved:true,productionCandidateCount,internalAuthorityRule:'HOME_TO_EXPLORE_AND_PRIMARY_HUBS; PRIMARY_HUBS_TO_CANDIDATE_DETAILS; VERIFIED_TOOLS_DIRECT_FROM_HOME; ALL_CANDIDATES_REACHABLE_WITHIN_3_CLICKS'};
...
const exploreDepth=distance.get('/explore/')??null;

manifest.uiVersion='11.20';
manifest.v11_20={internalAuthorityPolicy:true,budgetExplorerPreserved:true,productionCandidateCount,homeDiscoveryLinks:journeyRows.length,homePrimaryHubLinks:homeHubCoverage.filter(x=>x.linked).length,homeVerifiedToolLinks:homeDirectVerifiedTools.length,exploreDepth,maxCandidateDepth,orphanCandidateCount:orphanCandidates.length,hubCoverageComplete:Object.values(hubCoverage).every(row=>row.missing.length===0),qualityPolicyMetadataAligned:true,categoryDirectoryRowStyle:true};
await fs.writeFile(manifestPath,JSON.stringify(manifest,null,2),'utf8');

const report={schemaVersion:1,generatedAt,uiVersion:'11.20',previewMode:true,policy:'PRESERVE_V11_19_EXPLORE; HOME_LINKS_EXPLORE_PLUS_4_PRIMARY_HUBS_AND_8_VERIFIED_TOOLS; HUBS_COVER_ALL_CANDIDATE_DETAILS; CANDIDATE_DETAIL_DEPTH_LE_2; ALL_CANDIDATE_DEPTH_LE_3; NO_ORPHAN_CANDIDATES',productionCandidateCount,baselineBudgetExplorer:{productionCandidate:budgetIntent.productionCandidate,productionCandidateCount:budgetIntent.productionCandidateCount,trustedBrands:budgetIntent.trustedBrands},candidateTypeCounts,catalogHubCounts:{brandLinks:brandHubLinks.length,categoryLinks:categoryHubLinks.length,compareLinks:compareHubLinks.length,toolLinks:toolHubLinks.length},home:{discoveryLinks:journeyRows.map(row=>internalRoute(row.href)),primaryHubLinks:homeHubCoverage,verifiedToolLinks:homeDirectVerifiedTools.length,verifiedToolRoutes:homeDirectVerifiedTools,exploreLinked:homeLinks.has('/explore/')},graph:{htmlRouteCount:htmlByRoute.size,candidateHtmlMissing,unreachableCandidates,maxCandidateDepth,depthDistribution,orphanCandidates,hubDetailDepthViolations,allDepthViolations,exploreDepth},hubCoverage,qualityPolicyUpdated:{compare:quality.qualityPolicy.compare,tools:quality.qualityPolicy.tools,budget:quality.qualityPolicy.budget}};
await fs.writeFile(path.join(out,'v11-20-internal-authority.json'),JSON.stringify(report,null,2),'utf8');
console.log(JSON.stringify({v11_20:'PASS',productionCandidates:productionCandidateCount,candidateTypeCounts,homeVerifiedTools:homeDirectVerifiedTools.length,exploreDepth,maxCandidateDepth,orphanCandidates:orphanCandidates.length,hubCoverage:Object.fromEntries(Object.entries(hubCoverage).map(([k,v])=>[k,`${v.linkedCandidates}/${v.expected}`]))},null,2));
