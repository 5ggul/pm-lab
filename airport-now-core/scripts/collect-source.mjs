...
async function flights(){
  ...
  for(let attempt=1;attempt<=2;attempt++){
    ...
    try{
      ...
      await fetchFlightRows({provider,direction,serviceDate,serviceKey,numOfRows:1000},{maxPages:20,fetchImpl:url=>providerFetch(url,{signal:deadline}),capture:async page=>{if(page.httpStatus!==200)throw new Error('PROVIDER_HTTP_ERROR');pages.push(page.body);}});
