import { persistenceDecision } from './writer.js';
...
export async function persistFlightBatch(db,collapsed){
  const current=await existingRows(db,collapsed.flights);
  const plan=planFlightChanges(current,collapsed.flights);
  const queryCount=1+await persistBulk(db,{...plan,aliases:collapsed.aliases});
  return {...collapsed.diagnostics,changedFlights:plan.changedFlights.length,emittedEvents:plan.events.length,queryCount};
}
export const FLIGHT_BULK_SQL=Object.freeze({SELECT_EXISTING_SQL,UPSERT_CHANGED_SQL,INSERT_ALIASES_SQL,INSERT_EVENTS_SQL});
