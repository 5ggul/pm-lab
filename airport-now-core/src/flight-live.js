...
const STATUSES={'출발':'DEPARTED','이륙':'DEPARTED','도착':'ARRIVED','착륙':'LANDED','탑승중':'BOARDING','탑승':'BOARDING','결항':'CANCELLED','지연':'DELAYED','회항':'DIVERTED','정시':'SCHEDULED','예정':'SCHEDULED'};
...
export function normalizeKacFlight(row,ctx) {
  ...
  return normalize({flightNumber:row.flightid,master:row.masterflightid,
...
export function collapseFlightRows(rows,normalizeRow,ctx) {
  const flights=new Map(),aliases=new Map(),rejected=[];
  let duplicateRows=0;
  for(let i=0;i<rows.length;i++) {
    try {
      ...
      if(previous&&!alias)duplicateRows++;
      if(!previous||compare(rank,previous.rank)>0)flights.set(f.flightInstanceId,{rank,flight:canonical});
    }catch(error){rejected.push({index:i,reason:/^[A-Z_]+$/.test(error.message)?error.message:'MALFORMED_ROW'});}
  }
  if(rows.length&&!flights.size)throw new Error('ALL_FLIGHT_ROWS_REJECTED');
  return {flights:[...flights.values()].map(v=>v.flight),aliases:[...aliases.values()],
    diagnostics:{rawRows:rows.length,operatingFlights:flights.size,marketingAliases:aliases.size,duplicateRows,rejectedRows:rejected.length,rejected}};
}
