...
| IIAC passenger arrivals | data.go.kr `15095093`, `StatusOfPassengerFlightsOdp/getPassengerArrivalsOdp`, 1,000/day dev | documented adapter exists; GitHub Actions received the key but `apis.data.go.kr:443` timed out before an HTTP response, so live field verification is still pending | OFF |
...
| IIAC arrival-hall congestion | data.go.kr `15095061`, `StatusOfArrivals/getArrivalsCongestion` | raw waiting-count adapter ready; GitHub-hosted probe currently cannot reach `apis.data.go.kr:443` | OFF |
