...
(async()=>{
  ...
  try{
    ...
    const report={schema:'interior-final-compare-density-qa/v1',generatedAt:new Date().toISOString(),height:boot.height,passed:results.filter(r=>r.ok).length,failed:failures.length,results,failures,pageErrors,badResponses};
    await fs.writeFile(process.env.QA_RESULT_PATH||'final-density.json',JSON.stringify(report,null,2)+'\n','utf8');
    console.log('QA_TOTAL_ASSERTIONS='+results.length);
    console.log('QA_FAILURES='+failures.length);
    console.log('MOBILE_HEIGHT='+boot.height);
  }catch(err){
