...
const ORIGIN='https://5ggul.github.io';
...
function liveUrlForFile(file){
  ...
  if(rel==='index.html') return ORIGIN+PREFIX+'/';
  return ORIGIN+PREFIX+'/'+rel.replace(/index\.html$/,'');
}
...
function attrUrls(html,base){
  ...
  for(const m of html.matchAll(re)){
    ...
    try{
      ...
      u.hash='';
      if(u.origin===ORIGIN&&u.pathname.startsWith(PREFIX)) out.push(u.href);
    }catch{}
  }
  return out;
}
...
(async()=>{
  ...
  await mapLimit(files,16,async(file)=>{
    ...
    try{
      ...
      record('canonical absolute '+new URL(url).pathname,c.startsWith(ORIGIN+PREFIX+'/'),c||'(missing)');
  ...
  for(const u of sitemapUrls){
    ...
    const html=allHtml.get(norm)||allHtml.get(norm.replace(/\/$/,'/'));
    if(!html){
    ...
    record('public canonical matches sitemap URL '+new URL(u).pathname,c===norm,c);
  }
  ...
  const browserUrls=[...new Set([...sitemapUrls,...extraPaths.map(p=>ORIGIN+PREFIX+p)])];
  ...
  await Promise.all(Array.from({length:workerCount},async(_,wi)=>{
    ...
    page.on('response',r=>{
      if(r.status()>=400&&r.url().startsWith(ORIGIN+PREFIX)) badResponses.push({page:current,status:r.status(),url:r.url()});
    });
    while(true){
      const i=next++;
  ...
  const report={
    schema:'interior-final-sitewide-live-audit/v1',
