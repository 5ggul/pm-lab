...
(async()=>{
  ...
  try{
    ...
    const report={schema:'interior-p1-import-parser-hotfix-qa/v1',generatedAt:new Date().toISOString(),passed:results.filter(r=>r.ok).length,failed:failures.length,results,failures,pageErrors,badResponses,writeRequests};
    await fs.writeFile(process.env.QA_RESULT_PATH||'interior-p1-import-parser-hotfix-qa.json',JSON.stringify(report,null,2)+'\n','utf8');
    console.log('QA_TOTAL_ASSERTIONS='+results.length);
    console.log('QA_FAILURES='+failures.length);
    if(failures.length)process.exitCode=1;
  }catch(err){
    console.error(err?.stack||err);
    const report={schema:'interior-p1-import-parser-hotfix-qa/v1',generatedAt:new Date().toISOString(),passed:results.filter(r=>r.ok).length,failed:failures.length+1,results,failures:[...failures,{name:'fatal',detail:String(err?.stack||err)}],pageErrors,badResponses,writeRequests};
    await fs.writeFile(process.env.QA_RESULT_PATH||'interior-p1-import-parser-hotfix-qa.json',JSON.stringify(report,null,2)+'\n','utf8').catch(()=>{});
    process.exitCode=1;
  }finally{
    await browser.close();
