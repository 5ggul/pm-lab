...
const BASE='https://5ggul.github.io';
...
(async()=>{
  ...
  must(!sitemapLocs.includes(PREVIEW_CANON+'/region/'),'region hub remains held out of sitemap until owner cutover');
  ...
  must(schemaScripts>=69,'structured-data scripts parse across candidates',String(schemaScripts));
  ...
  try{
    ...
    for(const route of routes){
      ...
      must(res.ok(),'live HTTP 200',route+' :: '+res.status());
    }
