...
(async () => {
  ...
  try {
    ...
    await Promise.all([
      rp.waitForLoadState('domcontentloaded'),
    ...
    const report = {
      schema:'interior-v41-production-candidate-qa/v1',
    ...
    const report={schema:'interior-v41-production-candidate-qa/v1',generatedAt:new Date().toISOString(),baseUrl:BASE_URL,passed:results.filter((r)=>r.ok).length,failed:failures.length+1,results,failures:[...failures,{name:'fatal',detail:String(err?.stack || err)}],pageErrors};
    await fs.writeFile(OUT,JSON.stringify(report,null,2) + '\n','utf8').catch(()=>{});
