...
(async()=>{
  ...
  try{
    // Current production baseline: wizard hides all but one row on mobile.
    const baseContext=await browser.newContext({viewport:{width:375,height:800},hasTouch:true,isMobile:true,locale:'ko-KR'});
    const base=await baseContext.newPage();
    await goto(base,PROD+'/quote-check/');
    const prodBaseline=await base.evaluate(()=>({
    ...
    must(prodBaseline.total===12,'production baseline has 12 quote rows',JSON.stringify(prodBaseline));
    must(prodBaseline.visible===1&&prodBaseline.wizardClass,'production baseline defaults to one-row wizard',JSON.stringify(prodBaseline));
    await baseContext.close();

    // Candidate default all-row view.
    ...
    const report={
      schema:'interior-p1-quote-check-allrows-qa/v1',
      generatedAt:new Date().toISOString(),
      productionBaseline:prodBaseline,
      candidate:{height:boot.height,visibleRows:boot.visible},
      passed:results.filter(r=>r.ok).length,
      failed:failures.length,
      results,failures,pageErrors,badResponses,writes
    };
    await fs.writeFile(process.env.QA_RESULT_PATH||'interior-p1-quote-check-allrows-qa.json',JSON.stringify(report,null,2)+'\n','utf8');
    console.log('QA_TOTAL_ASSERTIONS='+results.length);
    console.log('QA_FAILURES='+failures.length);
    console.log('PRODUCTION_VISIBLE_ROWS='+prodBaseline.visible);
    console.log('CANDIDATE_VISIBLE_ROWS='+boot.visible);
    console.log('PRODUCTION_HEIGHT='+prodBaseline.height);
    console.log('CANDIDATE_HEIGHT='+boot.height);
  }catch(err){
